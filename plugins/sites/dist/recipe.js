...
const RECIPE_KINDS = ['release-copy', 'node-app'];
export const isRecipeKind = (value) => typeof value === 'string' && RECIPE_KINDS.includes(value);
...
const parseReadiness = (raw, image) => {
    if (raw === undefined)
        return { path: '/', expectStatus: image === 'static' ? 200 : 200 };
    if (raw === null || typeof raw !== 'object')
        throw new Error('recipe readiness must be an object');
    const record = raw;
    const path = record.path === undefined ? '/' : asString(record.path, 'readiness.path');
    if (!path.startsWith('/'))
        throw new Error('recipe readiness path must be absolute');
    if (path.includes('..'))
        throw new Error('recipe readiness path must not traverse');
...
export const staticServable = (rel) => {
...
const asString = (value, field) => {
    if (typeof value !== 'string' || value.trim() === '')
        throw new Error(`recipe ${field} must be a non-empty string`);
    return value;
};
...
export function parseAppRecipe(raw) {
    if (raw === null || typeof raw !== 'object')
        throw new Error('a recipe must be a JSON object');
    const record = raw;
    const kind = record.kind;
    if (!isRecipeKind(kind))
        throw new Error(`unknown recipe kind: ${String(kind)}`);
    if (!Array.isArray(record.argv) || record.argv.length === 0)
        throw new Error('recipe argv must be a non-empty array');
    ...
    if (!Array.isArray(secretsRaw))
        throw new Error('recipe secretFiles must be an array');
...
export function loadAppRecipe(artifactDir) {
...
export function auditStaticTree(workspace) {
    const offenders = [];
    ...
    const walk = (dir, prefix) => {
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (!entry.isFile()) {
                offenders.push(`${rel} is not a regular file`);
                continue;
            }
            if (!staticServable(rel))
                offenders.push(`${rel} must not be served by a static site`);
            ...
            try {
                ...
                if (real !== root && !real.startsWith(`${root}/`))
                    offenders.push(`${rel} resolves outside the workspace`);
            }
            catch {
...
export function relaxStaticServingPermissions(workspace, ancestors) {
...
export function installAppRecipe(artifactDir, input) {
...
export function recipeBinding(artifactDir) {
    const path = join(artifactDir, 'recipe.json');
    if (!existsSync(path))
        return null;
    try {
        const raw = JSON.parse(readFileSync(path, 'utf8'));
        if (typeof raw.siteId !== 'string' || typeof raw.expectedReleaseId !== 'string')
            return null;
...
export function provisionScript(recipe) {
    return [
        '#!/bin/sh',
        'set -eu',
        ...
        ...recipe.secretFiles.map((file) => `if [ -f '${join(SECRET_STAGE, file)}' ]; then mkdir -p "$(dirname '/workspace/${file}')"; install -m 0600 '${join(SECRET_STAGE, file)}' '/workspace/${file}'; fi`),
