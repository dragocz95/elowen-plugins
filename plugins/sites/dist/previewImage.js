...
export class SitePreviewImageService {
    ...
    unavailableReason(site) {
        ...
        if (!control)
            return 'the Browser plugin is not available on this instance, so nothing can render the page';
        if (!control.available())
            return 'this instance has no browser to render pages with';
        return null;
    }
    imagePath(siteId) {
    ...
    read(siteId) {
        ...
        if (!row || row.version === 0 || !row.mime)
            return null;
        const path = this.imagePath(siteId);
        try {
            const stat = statSync(path);
            if (!stat.isFile() || stat.size === 0 || stat.size > PREVIEW_IMAGE_MAX_BYTES)
                return null;
    ...
    request(siteId, cause) {
        const at = this.now();
        const site = this.deps.store.siteById(siteId);
        if (!site)
            return { ok: false, reason: 'that site no longer exists' };
        const reason = this.unavailableReason(site);
        if (reason)
            return { ok: false, reason };
        if (this.busy(siteId, at))
            return { ok: false, reason: 'a picture of this site is already being taken' };
        if (cause === 'manual') {
            const row = this.deps.store.previewImage(siteId);
    ...
    ensureFresh(sites, at = this.now()) {
        for (const site of sites) {
            if (this.busy(site.id, at))
                continue;
            const row = this.deps.store.previewImage(site.id);
            if (row && row.nextAttemptAt > at)
                continue;
            if (row && this.fresh(row, at))
                continue;
            if (this.unavailableReason(site))
                continue;
    ...
    async attempt(site) {
        ...
        this.deps.store.putCaptureGrant(grant.tokenHash, site.id, site.accessGeneration, this.now() + CAPTURE_GRANT_TTL_MS);
        try {
...
function checkCapturedImage(captured) {
    const bytes = captured.image;
    if (!(bytes instanceof Uint8Array) || bytes.byteLength === 0)
        throw new Error('the capture returned no image');
...
function writeImageFile(path, bytes) {
