...
export class EnvironmentSupervisor {
    ...
    async exportProject(site, project, guestPath, destinationPath, actor) {
        const artifact = { kind: 'project-source', project, guestPath, destinationPath: this.ownedPath(site.id, destinationPath) };
    ...
    async snapshot(site, input, actor) {
        ...
        if (!result.snapshotId)
            throw new Error(`snapshot operation ${result.id} completed without a snapshot ID`);
        ...
        if (!release)
            throw new Error('the completed snapshot is not present in the runtime snapshot inventory');
        return release;
    }
    ...
    async syncSnapshots(site, actor) {
        ...
        for (const snapshot of snapshots) {
            if (this.deps.store.release(site.id, snapshot.id))
                continue;
