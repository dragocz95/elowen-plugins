...
const proxyRefusal = (what) => `This publication is served by the environment of its Project, which owns no per-site lifecycle or logs. ${what}`;
const workdirOf = (value) => {
    if (value === undefined)
        return undefined;
...
const ended = (detail) => (/[.!?]$/.test(detail) ? detail : `${detail}.`);
...
const publishedAddressLines = (address, certificate) => {
    if (address === null)
        return [`The public hostname is unavailable: ${ended(certificate.detail)}`];
    if (certificate.state === 'ready')
        return [`Address: ${address}`, `Certificate: verified - ${ended(certificate.detail)}`];
...
const projectLines = (site, project) => {
    if (site.kind !== 'proxy')
        return [];
...
const describe = (site, config, environment, lastSnapshotAt, project) => {
...
export function registerTools(deps) {
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            ...
            if (site.kind === 'proxy')
                throw new ToolError(proxyRefusal('Run its commands in the Project environment with the Project shell and Sandbox tools.'));
            ...
            if (typeof command !== 'string' || command.length === 0 || command.includes('\0'))
                throw new ToolError('command must be non-empty text without NUL bytes.');
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            ...
            requireEnvironmentAuthority(deps, site, userId);
            if (site.kind === 'proxy')
                throw new ToolError(proxyRefusal('Start, stop and restart belong to that Project, in the Sandbox plugin.'));
            if (site.runtime !== 'environment')
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            ...
            requireEnvironmentAuthority(deps, site, userId);
            if (site.kind === 'proxy')
                throw new ToolError(proxyRefusal('A snapshot of that Project belongs to the Project, in the Sandbox plugin.'));
            if (site.runtime !== 'environment')
        ...
        execute: async (_id, input) => {
            try {
                ...
                if (site.runtime === 'unsupported')
                    throw new ToolError(`This site has an unsupported runtime: ${site.unsupportedRuntime ?? 'unknown'}.`);
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            try {
                ...
                const site = requireManaged(deps, input.site, userId);
                if (site.kind === 'proxy')
                    throw new ToolError(proxyRefusal('Read that Project environment\'s logs in the Sandbox plugin.'));
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            try {
                ...
                const site = requireManaged(deps, input.site, userId);
                if (site.kind === 'proxy')
                    throw new ToolError(proxyRefusal('Its output is the Project environment\'s journal, read in the Sandbox plugin.'));
                if (site.runtime === 'environment') {
                ...
                if (site.ownerUserId !== userId)
                    throw new ToolError('Only the site owner may read this runtime log.');
                if (site.runtime !== 'command')
