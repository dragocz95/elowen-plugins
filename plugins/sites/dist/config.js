...
export function environmentLimitOverrides(raw) {
    const out = {};
    for (const [key, bounds] of Object.entries(ENVIRONMENT_LIMITS)) {
        if (!(key in raw))
            continue;
        if (raw[key] === null) {
            out[key] = null;
...
export function resolveGatewayDnsTarget(value, fallbackHostname) {
    ...
    if (family === 4)
        return { target: { kind: 'ipv4', value: candidate }, error: null };
    if (family === 6)
        return { target: { kind: 'ipv6', value: candidate.toLowerCase() }, error: null };
