...
function phpTarget(releaseDir, rest) {
    ...
    if (!front || !existsSync(front) || !statSync(front).isFile())
        return null;
    return { script: front, scriptName: '/index.php', pathInfo: clean ? `/${clean}` : '' };
}
function parseCgi(stdout, siteRoot) {
    const crlf = stdout.indexOf('\r\n\r\n');
    const lf = stdout.indexOf('\n\n');
    const split = crlf >= 0 ? { at: crlf, width: 4 } : lf >= 0 ? { at: lf, width: 2 } : null;
    if (!split || split.at > HEADER_CAP_BYTES)
        throw new PhpError('PHP-CGI returned malformed headers');
    const rawHeaders = {};
    let status = 200;
    ...
    if (!explicitStatus && typeof rawHeaders.location === 'string')
        status = 302;
    return {
        status,
...
async function stopProcessGroup(pid) {
    ...
    while (Date.now() < killed && groupAlive(pid))
        await new Promise((resolve) => setTimeout(resolve, 20));
    if (groupAlive(pid))
        throw new PhpError('the PHP process group did not stop');
}
...
export async function executePhp(deps, site, releaseDir, req, rest, viewer, limits, siteRoot) {
    ...
    for (const [name, value] of Object.entries(req.headers)) {
        if (!CGI_REQUEST_HEADERS.has(name))
            continue;
    ...
    return new Promise((resolve, reject) => {
        ...
        heartbeat.unref?.();
        const releaseWhenStopped = async () => {
            try {
        ...
        const finish = async (error, response) => {
            if (settled)
                return;
            settled = true;
            ...
            try {
                ...
                cleanupError = cause instanceof Error ? cause : new PhpError(String(cause));
            }
            if (cleanupError) {
