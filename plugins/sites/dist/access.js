...
export const canManage = (site, userId, deps) => userId !== null && (deps.isAdmin(userId) || userId === site.ownerUserId);
...
export function mayOpen(site, viewer, store, deps) {
    ...
    if (userId === null)
        return false;
    if (!deps.accountExists(userId))
        return false;
    if (deps.isAdmin(userId))
        return true;
    if (userId === site.ownerUserId)
        return true;
    if (store.isMember(site.id, userId))
        return true;
    if (site.visibility === 'authenticated')
        return true;
    if (site.visibility === 'project')
        return deps.canAccessProject(userId, site.projectId);
    return false;
}
/** Whether an account may create and publish sites at all. Viewing is governed by the site itself. */
export function mayPublish(userId, deps, publishers) {
    if (userId === null)
        return false;
    if (!deps.accountExists(userId))
        return false;
...
export function normalizeReturnPath(raw) {
    ...
    if (segments.some((segment) => segment === '..' || segment === '.'))
        return '';
    if (segments[0] === RESERVED_PREFIX || decoded.split('/')[0] === RESERVED_PREFIX)
        return '';
