...
import { probeGatewayCertificate, retryDue } from './certificate.js';
import { verifyOwnershipTxt } from './dns.js';
const DNS_DELAYS_MS = [60_000, 2 * 60_000, 5 * 60_000, 15 * 60_000, 60 * 60_000];
const CERTIFICATE_DELAYS_MS = [60_000, 5 * 60_000, 15 * 60_000, 60 * 60_000];
const delayAt = (delays, failures) => delays[Math.min(failures, delays.length - 1)] ?? delays[delays.length - 1] ?? 60_000;
const dateAfter = (now, delay) => new Date(now + delay).toISOString();
export class SiteHostnameCoordinator {
    ...
    async issue(record) {
        ...
        if (!binding)
            return;
        if (!retryDue(record.certificateRetryAt, this.now()))
            return;
    ...
    async runCustomCheck(record, renew) {
        ...
        const [ownership, traffic] = await Promise.all([
            record.ownershipVerifiedAt === null
                ...
                : Promise.resolve({ state: 'ready', observedValues: [] }),
        ...
        if (!current || current.removalRequestedAt !== null)
            return;
    ...
    async issueGenerated(site, renew) {
        if (site.status !== 'live')
            return;
        const record = this.deps.store.generatedHostname(site.id);
        if (!record || record.removalRequestedAt !== null)
            return;
        ...
        if (!renew && !requested && this.deps.gateway.hasCertificate(record.hostname))
            return;
