...
import type { SiteEnvironmentAction, SiteEnvironmentOperation, SiteRuntimeArtifact, SiteImageKind, ManagedProjectRef } from 'elowen/plugin-api';
...
export interface EnvironmentDeps {
  ...
  buildSeedArchive(siteId: string, input: { provisionScript: string; appUnit: string; dataArchive: null }): Promise<string>;
}
...
const imageKindOf = (binding: Pick<SiteEnvironmentRegistration, 'image' | 'workspaceReadOnly'>): SiteImageKind =>
  binding.image === BASE_IMAGE_TAG ? 'base' : binding.workspaceReadOnly ? 'static' : 'node';
...
export class EnvironmentSupervisor {
  private readonly authority: SeededSiteRuntimeAuthority;
  ...
  constructor(private readonly deps: EnvironmentDeps) {
    this.authority = createSiteRuntimeAuthority({
      ...
      containerSeed: async id => {
        ...
        if (!existsSync(join(artifacts, 'recipe.json'))) return null;
  ...
  async exportProject(site: Site, project: ManagedProjectRef, guestPath: string, destinationPath: string, actor: number): Promise<void> {
    ...
    if (!this.registration(site.id)) this.saveBinding({ ...await this.defaultBinding(site), sourcePath: destination,
  ...
  endpointFor(id: string): Endpoint | null {
    const site = this.deps.store.siteById(id);
    if (!site || site.runtime !== 'environment') return null;
    const path = join(this.brokerDirectory(id), 'app.sock');
    try { return lstatSync(path).isSocket() ? { kind: 'socket', path } : null; }
    catch { return null; }
  }
  isRunning(id: string): boolean { return this.endpointFor(id) !== null; }
  ...
  async provision(site: Site, imageKind: SiteImageKind): Promise<void> {
    if (this.registration(site.id)?.persistentRootfs) throw new Error('persistent rootfs Sites materialize directly from their registered image');
  ...
  async prepareContainer(site: Site, workspace: string, image = BASE_IMAGE_TAG, workspaceReadOnly = false): Promise<{ created: boolean }> {
    ...
    if (!binding.persistentRootfs) await this.provision(site, imageKindOf(binding));
  ...
  async rebindToSource(site: Site, operationId: string = randomUUID()): Promise<void> {
    ...
    if (!binding.persistentRootfs) await this.perform(site, { kind: 'provision-image', imageKind: imageKindOf(binding) }, undefined, true, `${operationId}-image`);
  ...
  publishBinding(site: Site): void {
    const binding = this.registration(site.id);
    if (!binding) throw new Error('this site has no registered environment binding to publish');
  ...
  async clearConversionStage(site: Site, stageDir: string): Promise<void> {
    const result = await this.exec(site, `rm -rf -- '${stageDir}'`, { timeoutSeconds: 60 });
    if (result.code !== 0) throw new Error(`the conversion seed directory could not be cleared: ${result.stderr || result.stdout}`);
  }
  async conversionStageAbsent(site: Site, stageDir: string): Promise<boolean> {
  ...
  async stop(id: string): Promise<void> { await this.perform(this.site(id), { kind: 'stop' }); }
  async quiesce(id: string): Promise<void> { await this.perform(this.site(id), { kind: 'stop' }, undefined, true); }
  ...
  private async awaitReadiness(
    deadlineMs: number,
    attempt: () => Promise<{ done: boolean; ready: boolean; detail: string }>,
    ...
    do {
      ...
      if (result.done) return { ready: result.ready, detail, attempts };
  ...
  async probeReadiness(id: string, expect: { path: string; expectStatus: number }, options: { timeoutMs?: number; deadlineMs?: number } = {}): Promise<{ ready: boolean; detail: string; attempts: number }> {
    return this.awaitReadiness(options.deadlineMs ?? this.deps.config().startTimeoutSeconds * 1000, async () => {
      ...
      if (!endpoint) return { done: false, ready: false, detail: 'the environment ingress socket is unavailable' };
      ...
      if (typeof result === 'number') return { done: true, ready: result === expect.expectStatus, detail: `GET ${expect.path} answered ${result}, expected ${expect.expectStatus}` };
  ...
  async reconcile(): Promise<void> {
    for (const site of this.deps.store.environmentSitesForReconcile()) {
      if (this.deps.store.conversionSuspends(site.id) === 'environment') continue;
      try {
  ...
  async detach(): Promise<void> { this.connected = undefined; }
}
