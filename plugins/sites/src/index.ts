...
export function register(published: PluginContext): void {
  ...
  const sessionSecret = (): string => {
    if (cachedSecret === null) cachedSecret = mintSessionSecret();
    return cachedSecret;
  };
  ...
  const projectSlug = (projectId: number): string | null => ctx.host.stores().projects.get(projectId)?.slug ?? null;
  ...
  const cleanupDeletingSites = async (): Promise<void> => {
    ...
    for (const site of store.deletingSites()) {
      try { await deleteSite(site.id); } catch { /* durable marker retains retry ownership */ }
    }
  };
  const cleanupOrphans = async (): Promise<void> => {
    ...
    for (const site of store.allSites()) {
      ...
      try { await deleteSite(site.id); } catch { /* keep sweeping independent sites */ }
    }
  ...
  ctx.registerInterval('recover-site-gateway', async () => {
    if (!gateway.isActive()) await syncGateway();
  }, GATEWAY_RECOVERY_MS);
