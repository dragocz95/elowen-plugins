...
import { resolveConfig, siteHost, siteUrl, type SitesConfig } from './config.js';
...
export function register(published: PluginContext): void {
  ...
  const migration = new RuntimeMigrationService({
    ...
    restoreLegacyData: (selection, archive, siteId, archivePrefix) =>
      dataSync.restoreLegacyData(selection, archive, siteId, archivePrefix),
  ...
  const settleConversions = async (): Promise<void> => {
    ...
    for (const settled of await migration.reconcileRollbacks()) {
      if (settled.stage === 'none') ctx.logger.info(`site conversion ${settled.siteId} rolled back`);
      else ctx.logger.warn(`site conversion ${settled.siteId} rollback is pending: ${settled.lastError ?? settled.stage}`);
    }
