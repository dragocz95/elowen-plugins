...
export async function ensureBaseImage(podman: PodmanClient, dataDir: string): Promise<string> {
  if (await podman.imageExists(BASE_IMAGE_TAG)) return BASE_IMAGE_TAG;
