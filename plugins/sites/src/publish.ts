  ...
  const walk = (dir: string, target: string): void => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isSymbolicLink()) {
        ...
        let link: string;
        try { link = readlinkSync(from); }
        catch { warnings.push(`skipped symlink ${rel} (it changed while publishing)`); continue; }
        if (isAbsolute(link)) { warnings.push(`skipped symlink ${rel} (absolute target)`); continue; }
        ...
        if (fileCount >= fileCeiling) throw new PublishError(`the output has more than ${fileCeiling} files.`);
        symlinkSync(link, to);
        fileCount += 1;
        sizeBytes += linkBytes;
        continue;
      ...
      try {
        ...
        try {
          ...
          while (read < opened.size) {
            ...
            while (written < chunk) written += writeSync(out, buffer, written, chunk - written);
            read += chunk;
          }
        } finally {
          closeSync(out);
...
export function pruneReleases(store: SitesStore, siteId: string, siteDir: string, keep: number, currentReleaseId: string | null): void {
...
export function resolveWithin(releaseDir: string, relPath: string): string | null {
