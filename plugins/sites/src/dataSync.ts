...
/** A directory the sandbox actually owns for this site, resolved fresh per operation. */
export interface LegacyDataLocation {
  /** Absolute HOME the confined process was given. */
  home: string;
  /** The roots the sandbox granted alongside it, used to prove `home` is one of them. */
...
export function validateLegacyLocation(location: LegacyDataLocation): string {
  ...
  if (roots.length === 0) throw new Error('the sandbox reported no roots, so HOME cannot be verified as owned');
  ...
  if (existsSync(home) && !statSync(home).isDirectory()) throw new Error('the sandbox HOME is not a directory');
  return resolve(home);
}
