...
import { SiteDomainError, type SiteDomainService } from './domains.js';
...
export interface ApiDeps {
  ...
  access: AccessDeps;
  config(): SitesConfig;
  addresses: Pick<SiteAddressService, 'urlForSite' | 'bindingById' | 'urlForHostname'>;
...
export function createApiHandlers(deps: ApiDeps) {
  ...
  const site = async (req: PluginApiRequest): Promise<PluginHttpResponse> => {
    ...
    if (action === 'domains') {
      ...
      try {
        if (req.method === 'GET' && domainId === '') return json(200, await deps.domains.list(target));
        if (req.method === 'POST' && domainId === '') {
          const body = await req.json<{ hostname?: unknown }>().catch(() => ({} as { hostname?: unknown }));
    ...
    if (req.method === 'POST' && action === 'rollback') {
      if (target.kind === 'proxy') return json(409, { error: 'a proxy publication has no file releases' });
      const body = await req.json<{ releaseId?: unknown }>().catch(() => ({} as { releaseId?: unknown }));
      const releaseId = typeof body.releaseId === 'string' ? body.releaseId : '';
      const release = deps.store.release(target.id, releaseId);
      if (!release || release.kind === 'environment-snapshot') return json(404, { error: 'unknown release' });
  ...
  const ticket = async (req: PluginApiRequest): Promise<PluginHttpResponse> => {
    ...
    const body = await req.json<{ binding?: unknown; r?: unknown }>()
      .catch(() => ({} as { binding?: unknown; r?: unknown }));
    const bindingId = typeof body.binding === 'string' ? body.binding : '';
    const binding = deps.addresses.bindingById(
      deps.store.hostnameById(bindingId)?.siteId
        ?? (bindingId.startsWith('preview:') ? bindingId.slice('preview:'.length) : ''),
      bindingId,
    );
