...
type GatewayDnsCheck = Omit<SiteGatewayReadiness, 'id' | 'label'>;
...
const unavailableStatus = (detail: string, hostnameBase: string | null): SitesGatewayStatus => ({
  available: false,
  active: false,
  hostnameBase,
  detail,
...
export class SiteGatewayManager {
  private status: SitesGatewayStatus | null = null;
  private inFlight: Promise<SitesGatewayStatus> | null = null;
  private readonly destination: GatewayDnsTargetService;
  private readonly randomLabel: () => string;

  constructor(private readonly ctx: SitesContext, deps: GatewayDeps = {}) {
    const resolver = deps.resolver ?? new Resolver({ timeout: 5_000, tries: 2 });
    const trafficResolver: TrafficDnsResolver = {
  ...
  private appHostname(): string | null {
    const web = this.ctx.publicWebUrl();
    if (!web) return null;
    try { return new URL(web).hostname.toLowerCase(); } catch { return null; }
  }
  ...
  bindingStatuses(): readonly SitesGatewayBindingStatus[] {
  ...
  private async checkGatewayDns(): Promise<GatewayDnsCheck> {
    const base = this.hostnameBase();
  ...
  async verifyHostnameDns(hostname: string) {
    const resolved = this.destination.current();
    if (!resolved.target) {
      return { state: 'unavailable' as const, observedTargets: [], detail: resolved.error ?? 'The Sites DNS destination is unavailable.' };
    }
  ...
  private async reconcileOnce(bindings: readonly SitesGatewayBinding[]): Promise<SitesGatewayStatus> {
  ...
  async ensureBinding(binding: SitesGatewayBinding, bindings: readonly SitesGatewayBinding[]): Promise<SitesGatewayStatus> {
    const control = this.control();
    if (!control) throw new Error('No published-sites gateway broker is available in this process.');
    const email = typeof this.ctx.config.contactEmail === 'string' ? this.ctx.config.contactEmail.trim() : '';
    if (!email) throw new Error('A contact email is required before a site certificate can be issued.');
    ...
    this.status = status;
    if (!status.available || !status.active) throw new Error(status.detail ?? `The gateway did not activate ${binding.hostname}.`);
    return status;
  }
  ...
  async removeBinding(
    ...
    this.status = status;
    if (!status.available) throw new Error(status.detail ?? `The gateway did not remove ${hostname}.`);
    return status;
  }

  async readiness(): Promise<SiteGatewayReadiness> {
    const row = { id: 'sites-gateway' as const, label: 'Published sites gateway' };
    const dns = await this.checkGatewayDns();
    if (!dns.ok) return { ...row, ...dns };
