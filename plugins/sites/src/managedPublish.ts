...
type GuestEntryKind = 'file' | 'directory' | 'symlink';

interface GuestWalkResult {
  kind: 'walk';
  root: string;
  rootKind: GuestEntryKind | 'other' | null;
  entries: { path: string; kind: GuestEntryKind; size: number; mtime: number }[];
  truncated: boolean;
}
...
interface GuestExportManifestResult {
  kind: 'export-manifest';
  root: string;
  mode: number;
  entries: GuestExportEntry[];
}

type GuestProjectFileResult = GuestWalkResult | GuestReadResult | GuestExportManifestResult | { kind: 'mkdir'; entry: unknown };
...
export interface ManagedProjectFiles {
  ...
  }): Promise<GuestProjectFileResult>;
}
...
const manifestPath = (path: unknown): string => {
...
const permissionMode = (mode: unknown): number => {
...
const symlinkTarget = (path: string, target: unknown): string => {
...
async function copyManagedFile(
  ...
  input: { projectId: number; accountUserId: number; limits: SnapshotLimits },
  sourcePath: string,
  target: string,
  rel: string,
  metadata: { size: number; version?: string; mode: number },
  sizeBefore: number,
): Promise<number> {
  mkdirSync(dirname(target), { recursive: true });
  const output = openSync(target, 'wx', 0o600);
  let offset = 0;
  let version: string | null = metadata.version ?? null;
...
function validateApplicationManifest(result: GuestExportManifestResult): GuestExportEntry[] {
  ...
  const entries: GuestExportEntry[] = [];
  const byPath = new Map<string, GuestExportEntry>();
  for (const raw of result.entries) {
    ...
    if (byPath.has(path)) throw new PublishError(`Sandbox returned duplicate publication metadata for ${path}.`);

    let entry: GuestExportEntry;
    if (raw.kind === 'file') {
...
async function snapshotManagedApplication(
  control: ManagedProjectFiles,
  input: { projectId: number; accountUserId: number; sourceRoot: string; releaseDir: string; limits: SnapshotLimits },
  ...
  if (result.kind !== 'export-manifest') throw new PublishError('Sandbox returned an invalid application manifest.');
  ...
  for (const entry of directories) mkdirSync(join(input.releaseDir, ...entry.path.split('/')), { recursive: true });
  for (const entry of files) {
    if (fileCount >= MAX_APPLICATION_FILES) throw new PublishError(`the output has more than ${MAX_APPLICATION_FILES} files.`);
    const target = join(input.releaseDir, ...entry.path.split('/'));
    const copied = await copyManagedFile(
      control,
      input,
      posix.join(input.sourceRoot, entry.path),
      target,
      entry.path,
      { size: entry.size!, version: entry.version, mode: entry.mode },
      sizeBytes,
    );
    fileCount += 1;
    sizeBytes += copied;
...
export async function snapshotManagedRelease(
  ...
  try {
    if (application) return await snapshotManagedApplication(control, input);
    ...
    if (walked.truncated) throw new PublishError(`the output has more than ${MAX_STATIC_FILES} entries.`);
    ...
    for (const entry of walked.entries) {
      ...
      if (fileCount >= MAX_STATIC_FILES) throw new PublishError(`the output has more than ${MAX_STATIC_FILES} files.`);
      if (!Number.isSafeInteger(entry.size) || entry.size < 0) throw new PublishError(`Sandbox returned inconsistent metadata for ${rel}.`);
