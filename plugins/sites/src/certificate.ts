...
/** `endpoint` overrides where the handshake goes. Only a test supplies one; production always asks the
 *  gateway on this machine, whose address is not configurable and must not become so. */
...
export const probeGatewayCertificate: GatewayCertificateProbe = (hostname, endpoint) => new Promise((resolve) => {
  let settled = false;
