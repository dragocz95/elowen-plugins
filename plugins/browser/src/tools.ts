...
export function registerBrowserTools(ctx: PluginContext, registry: SessionRegistry): void {
  ...
  const openBrowser = async (toolCallId: string, url: string | undefined, signal: AbortSignal | undefined) => {
    const owner = requireBrowserToolOwner(ctx);
  ...
  const tools = [
    defineTool({
      ...
      execute: (toolCallId: string, input: { url?: string }, signal?: AbortSignal) => openBrowser(toolCallId, input.url, signal),
    }),
    defineTool({
