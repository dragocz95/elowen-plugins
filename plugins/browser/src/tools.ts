...
export function registerBrowserTools(ctx: PluginContext, registry: SessionRegistry): void {
  ...
  const session = async (sessionId: string) => {
    ...
    if (!selected) throw new Error('The project browser session belongs to a managed project that is not selected.');
  ...
  const tools = [
    ...
    defineTool({
      ...
      execute: async (_toolCallId: string, input: { sessionId: string }, signal?: AbortSignal) => {
        const current = await session(input.sessionId);
        if (current.project) throw new Error('Interactive takeover is available only for personal browser sessions. Project browsers are controlled through browser tools.');
