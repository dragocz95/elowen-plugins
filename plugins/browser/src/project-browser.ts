...
import type { GuestFileOperation, GuestFileResult, GuestFileStat, PluginContext, SandboxPreparedExecution } from 'elowen/plugin-api';
...
export async function openProjectBrowser(ctx: PluginContext, project: BrowserProject, actor: number, logger: BrowserLogger): Promise<ProjectBrowserAttachment> {
  ...
  const guest = async (operation: GuestFileOperation): Promise<GuestFileResult> =>
    sandbox.projectFiles({ project, accountUserId: actor, expectedGeneration: generation, operation });
  const statEntry = async (path: string): Promise<GuestFileStat | null> => {
    const result = await guest({ kind: 'stat', path });
    if (result.kind !== 'stat') throw new Error(`Project browser storage returned ${result.kind} for a stat.`);
    return result.entry;
  };
  for (const path of ['/data/browser', '/data/browser/downloads']) {
    let entry = await statEntry(path);
    if (!entry) {
      try { await guest({ kind: 'mkdir', path }); }
      catch (error) {
    ...
    if (entry?.kind !== 'directory') throw new Error(`Project browser storage "${path}" is not a directory.`);
  }
...
async function connectProjectBrowser(ctx: PluginContext, prepared: SandboxPreparedExecution, project: BrowserProject, actor: number,
  ...
  try {
    ...
    if (prepared.stdin !== undefined) child.stdin.write(prepared.stdin);
