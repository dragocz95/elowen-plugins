    ...
    return this.queue.run(async () => {
      ...
      await withDeadline(
        this.input.dispatchUserBatch(events, () => {
          if (this.inputEpochValue !== epoch) throw new Error('Browser input was superseded before it finished.');
        }),
        10_000,
        'Browser input was not acknowledged in time.',
      );
