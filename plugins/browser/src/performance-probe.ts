...
export interface TraceSummary {
  /** How long the trace actually ran, measured by the recorder between start and stop. */
  durationMs: number;
  events: number;
  truncated: boolean;
  /** The span the recorded events cover, present ONLY when it is consistent with `durationMs`. Absent
   *  means the trace's own timestamps could not be reconciled with the measured run — which says
   *  something about the timestamps, not about the trace, so nothing is invented in its place. */
  eventSpanMs?: number;
...
export class TraceRecorder {
  ...
  private active: { sessionId: string; startedAt: number; startedAtMonotonic: number } | null = null;
