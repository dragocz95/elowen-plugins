        ...
        return {
          sse: async (send, signal) => {
            ...
            try {
              ...
              if (reason === 'stream_failed') ctx.logger.warn(`browser live view stream for ${session.id} failed: ${message}`);
              throw error;
            } finally {
  ...
  ctx.registerApiRoute({
    path: 'input', method: 'POST', access: 'user', handler: async (req) => {
      try {
        ...
        const outcome = await ownedSession(registry, req).dispatchUserInput(
  ...
  ctx.registerApiRoute({
    path: 'navigation', method: 'POST', access: 'user', handler: async (req) => {
      try {
        ...
        if (action !== 'back' && action !== 'forward' && action !== 'reload') throw new BrowserAccessError('Browser navigation action is invalid.', 400);
