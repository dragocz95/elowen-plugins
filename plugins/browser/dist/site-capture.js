...
export function validateCaptureUrl(raw) {
    ...
    if (url.hostname === '')
        throw new CaptureRefused('The capture URL names no host.');
    return url;
}
...
const puppeteerLauncher = async ({ executablePath, userDataDir, allowedHost }) => {
        ...
        async capture(request) {
            ...
            try {
                ...
                try {
                    ...
                    opened.browser?.process?.()?.kill('SIGKILL');
                }
                try {
                    rmSync(profile, { recursive: true, force: true });
                }
                catch (error) {
                    deps.logger.warn(`capture profile ${profile} was left behind: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        },
    };
