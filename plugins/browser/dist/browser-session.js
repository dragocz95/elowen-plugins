...
import { captureElement, captureFullPage, captureModelScreenshot, captureThumbnail, captureViewport, SCREENSHOT_JPEG_QUALITY, } from './capture.js';
...
export class BrowserSession {
    ...
    async navigate(url, signal) {
        return this.agentMutation(signal, async () => {
            ...
            if (this.deps.projectAuthority) {
                ...
                if (!['http:', 'https:'].includes(validated.protocol) || validated.username || validated.password)
                    throw new Error('Project browser requires an http(s) URL without credentials.');
    ...
    async screenshot(area, format, ref, signal) {
        return this.runAgentOperation(signal, async () => {
            ...
            if (area === 'fullPage')
                return captureFullPage(this.cdp, format, quality);
            if (!ref)
                throw new Error('An element ref is required to capture an element.');
            const snapshot = await this.ensureSnapshot();
            const element = snapshot.elements.get(ref);
            if (!element)
                throw new Error(`Accessibility element ${ref} was not found in the latest snapshot.`);
    ...
    async evaluate(expression, timeoutMs, awaitPromise, signal) {
        return this.runAgentOperation(signal, async () => {
    ...
    async networkRequest(requestId, includeBody, signal) {
        return this.runAgentOperation(signal, async () => {
            const entry = this.diagnostics.request(requestId);
            if (!entry)
                throw new Error('That request is not in this session\'s recent network buffer.');
            if (!includeBody)
                return { entry };
    ...
    async performanceMetrics(signal) {
    ...
    async stopTrace(signal) {
        return this.runAgentOperation(signal, () => this.tracer.stop(this.id));
    }
    get traceRunning() { return this.tracer.running; }
    ...
    async audit(includeScreenshot, signal) {
        return this.runAgentOperation(signal, async () => {
            const snapshot = await this.ensureSnapshot();
            return {
    ...
    close(reason = 'closed') {
        ...
        this.closedPromise = this.closeSession(reason).catch((error) => {
    ...
    refreshFavicon(url, generation, cdp) {
        void readPageFavicon(cdp).then(async (favicon) => {
            if (!favicon || generation !== this.faviconGeneration || cdp !== this.cdp || this.page.url() !== url)
                return;
            if (this.stateValue === 'closing' || this.stateValue === 'closed' || this.stateValue === 'error')
                return;
