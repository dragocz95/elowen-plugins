    ...
    return (hostname, options, callback) => {
        ...
        queueMicrotask(() => {
            ...
            if (candidates.length === 0) {
                const error = new Error('Proxy DNS lookup found no approved address for the requested family.');
                error.code = 'ENOTFOUND';
                callback(error);
                return;
            }
