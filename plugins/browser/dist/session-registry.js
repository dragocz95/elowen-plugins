...
export class SessionRegistry {
    ...
    create(input) {
        return this.createQueue.run(async () => {
            ...
            try {
                ...
                if (cleanupFailure)
                    throw new AggregateError([error, cleanupFailure], 'Browser creation and cleanup failed.');
                throw error;
            }
        });
    }
    ...
    async closeUser(ownerUserId, reason = 'user_removed') {
        ...
        if (failures.length)
            throw new AggregateError(failures, `Browser cleanup for account ${ownerUserId} was incomplete.`);
    }
    ...
    async closeAll(reason = 'plugin_reload') {
        ...
        const failures = results.filter((result) => result.status === 'rejected').map((result) => result.reason);
        if (failures.length)
            throw new AggregateError(failures, 'Browser shutdown was incomplete.');
    }
    async bootReconcile() {
        const now = this.deps.clock.now();
