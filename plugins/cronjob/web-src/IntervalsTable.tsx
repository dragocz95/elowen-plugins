...
export function IntervalsTable({ rows, jobs, query, onQueryChange, onOpen, onRun }: {
  ...
  return (
    <section className="flex min-w-0 flex-col gap-3" data-testid="cron-intervals-table">
      <header className="flex min-w-0 flex-wrap items-end justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <C.Badge tone="muted">{filtered.length}</C.Badge>
          </div>
      ...
      {filtered.length === 0 ? <C.EmptyState title={s.historyEmpty || 'No matching jobs'} /> : (
        <C.DataTable
            ...
            return (
              <C.DataTableRow key={row.jobId} height="tall" onOpen={() => onOpen(row.jobId)} openLabel={(s.openJob || 'Open “{name}”').replace('{name}', job.name)}>
                <C.DataTableCell lines="auto">
                  ...
                  <span className="flex items-center gap-1 text-[11px] text-muted-foreground"><C.Avatar size="xs" name={owner} src={job.owner?.avatar || undefined} />{owner}</span>
                </C.DataTableCell>
                ...
                <C.DataTableCell lines="auto" priority="wide"><span className="flex items-center gap-1"><C.Avatar size="xs" name={owner} src={job.owner?.avatar || undefined} />{owner}</span></C.DataTableCell>
                <C.DataTableCell lines="auto">
