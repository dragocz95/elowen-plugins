...
export class MissionStore {
    ...
    create(m) {
        this.db.prepare(`INSERT INTO missions (id,epic_id,autonomy,max_sessions,state,created_by,pilot_exec,overseer_exec)
    ...
    get(id) {
        return this.db.prepare(`SELECT ${COLS} FROM missions WHERE id=?`).get(id) ?? null;
    }
    active() {
        return this.db.prepare(`SELECT ${COLS} FROM missions WHERE state='active'`).all();
    }
    ...
    activeForEpic(epicId) {
        return this.db.prepare(`SELECT ${COLS} FROM missions WHERE state='active' AND epic_id=?`).get(epicId) ?? null;
    }
    ...
    live() {
        return this.db.prepare(`SELECT ${COLS} FROM missions WHERE state IN ('active','stalled')`).all();
    }
    setState(id, state) {
