...
import { buildAgentsRuntime, AGENTS_INTERVAL_MS } from './runtime.js';
...
export function register(ctx) {
    ...
    const tasksAvailable = () => ctx.host.stores().tasksAvailable();
    ...
    const rt = () => {
        ...
        if (!tasksAvailable())
            throw new TaskDomainUnavailableError();
        if (!runtime) {
    ...
    let stopDeriver = null;
    ctx.registerService({
        name: 'deriver',
        start: () => { if (!idleWithoutTasks())
            stopDeriver = rt().deriver.start(); },
    ...
    ctx.registerBootReconcile(() => { if (!idleWithoutTasks())
        rt().reconcileZombies(); });
    ctx.registerBootReconcile(() => { if (!idleWithoutTasks())
        rt().reconcileOverseers(); });
    ...
    for (const [name, ms] of Object.entries(AGENTS_INTERVAL_MS)) {
        ctx.registerInterval(name, () => { if (!idleWithoutTasks())
            rt().intervals.find((i) => i.name === name)?.fn(); }, ms);
    }
    ...
    ctx.registerEventProjectResolver((e) => {
        if (e.type === 'signal') {
        ...
        if (e.type === 'plan')
            return runtime?.planJobs.get(e.jobId)?.projectId ?? null;
        return null;
    });
    ...
    for (const tool of AGENTS_MCP_TOOLS)
    ...
    ctx.registerControl('missions', {
        ...
        advisor: () => ({
            ensureOnLogin: async (userId) => { await rt().advisor()?.ensureOnLogin(userId); },
            stop: async (userId) => { await rt().advisor()?.stop(userId); },
        }),
