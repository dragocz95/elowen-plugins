import { buildAgentCommand } from './commandBuilder.js';
...
export class SpawnService {
    ...
    attachBrainWorker(w) { this.d.brainWorker = w; }
    async launch(input) {
        ...
        if (input.spec.program === 'elowen') {
            if (!this.d.brainWorker)
                throw new Error('elowen exec engine not available (brain not configured)');
            if (input.rawPrompt)
                throw new Error('elowen exec engine does not support pilot/overseer raw prompts');
        ...
        const tokenFor = (e) => e.tokenForTask?.(input.taskId) ?? e.token;
        ...
        if (resume)
            log.info(`resuming ${resume.program} session ${resume.sessionId} for task ${input.taskId}`);
        ...
        const renderPrompt = (name, vars) => this.d.prompts.render(name, vars, input.ownerId);
        ...
        try {
            ...
            const reason = token ? String(e?.message ?? e).split(token).join('***') : String(e?.message ?? e);
