...
export const AGENTS_MCP_TOOLS = [
    ...
    },
    {
        ...
        run: (a, req) => req('GET', `/notes?scope=mission&target=${encodeURIComponent(a.target)}`),
    },
    // ---- Mission lifecycle ----
    {
    ...
    },
    {
        ...
        run: (a, req) => req('PATCH', `/missions/${encodeURIComponent(a.id)}`, { action: 'pause' }),
    },
    {
        ...
        inputSchema: { id: z.string() },
        run: (a, req) => req('PATCH', `/missions/${encodeURIComponent(a.id)}`, { action: 'resume' }),
    },
    {
        name: 'elowen_mission_disengage',
        description: 'Disengage (stop) a mission entirely, tearing down its agents. `id` is the mission id.',
        inputSchema: { id: z.string() },
        run: (a, req) => req('DELETE', `/missions/${encodeURIComponent(a.id)}`),
    },
    ...
    },
    {
        ...
        run: (a, req) => req('DELETE', `/sessions/${encodeURIComponent(a.name)}`),
    },
    {
        ...
        run: (a, req) => req('POST', `/sessions/${encodeURIComponent(a.name)}/keys`, { keys: a.keys }),
    },
    {
        ...
        run: (a, req) => req('GET', `/sessions/${encodeURIComponent(a.name)}/pane${a.ansi ? '?ansi=1' : ''}`),
    },
];
