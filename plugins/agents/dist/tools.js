...
export function registerAgentsTools(ctx, rt) {
    ...
    const denied = () => !ctx.currentAccess().owner;
    ...
    const unavailable = { content: [{ type: 'text', text: MISSIONS_WITHOUT_TASKS }], details: {} };
    const withoutTasks = () => !ctx.host.stores().tasksAvailable();
    ctx.registerTool(defineTool({
        ...
        execute: async () => {
            ...
            if (withoutTasks())
                return unavailable;
            return { content: [{ type: 'text', text: JSON.stringify(missionsListPayload(rt(), ctx.host.stores().tasks, auth())) }], details: {} };
        },
    }));
    ctx.registerTool(defineTool({
        ...
        execute: async () => {
            ...
            if (withoutTasks())
                return unavailable;
            return { content: [{ type: 'text', text: JSON.stringify(await sessionsListPayload(ctx.host.tmux(), rt, auth())) }], details: {} };
        },
    }));
}
