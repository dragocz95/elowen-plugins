...
import { buildReview, buildNeedsInput, buildStalled, buildBlocked, buildDone } from './messages.js';
...
export class PushDispatcher {
    ...
    handle(e) {
        ...
        if (recipients.length === 0)
            return; // no one to notify (e.g. owner-less mission, no admins)
    ...
    map(e) {
        if (e.type === 'review') {
            if (e.approve)
                return null; // approved → nothing to decide
        ...
        if (e.type === 'mission' && e.state === 'disengaged') {
            ...
            if (!epicId || this.d.tasks.get(epicId)?.status !== 'closed')
                return null;
