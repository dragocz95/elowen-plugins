...
export class DecisionQueue {
    now;
    queues = new Map(); // missionId → FIFO of unanswered requests
    waiters = new Map(); // missionId → long-poll resolvers awaiting a request
    /** `now` is injectable so the sweep and tests can drive `enqueuedAt` deterministically. */
    constructor(now = Date.now) {
        this.now = now;
    }
    enqueue(missionId, kind, context) {
        return new Promise((resolveVerdict) => {
            const id = randomBytes(6).toString('hex');
            const entry = { id, kind, context, enqueuedAt: this.now(), settle: resolveVerdict };
    ...
    pending() {
        ...
        for (const [missionId, list] of this.queues) {
            for (const e of list)
                out.push({ missionId, id: e.id, kind: e.kind, enqueuedAt: e.enqueuedAt });
        }
        return out;
    }
    ...
    drain(missionId) {
        for (const e of this.queues.get(missionId) ?? [])
            e.settle({ approve: false, confidence: 0, rationale: 'mission disengaged' });
        this.queues.delete(missionId);
        for (const w of this.waiters.get(missionId) ?? [])
            w(null);
        this.waiters.delete(missionId);
    }
    wakeWaiter(missionId) {
        const w = (this.waiters.get(missionId) ?? []).shift();
        const head = (this.queues.get(missionId) ?? [])[0];
        if (w && head)
            w({ id: head.id, kind: head.kind, context: head.context });
    }
    dropWaiter(missionId, w) {
