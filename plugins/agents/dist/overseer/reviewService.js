...
export function createReviewService(d) {
    ...
    const checkoutPathFor = (missionId, projectId) => d.missionGit.worktreeFor(missionId) ?? d.pathFor(projectId);
    ...
    async function onTaskClosed(id, existing, opts) {
        ...
        if (mission && cfg.reviewOnDone && (mission.overseer_exec || cfg.overseerExec)) {
            ...
            for (const e of d.tasks.allDeps()) {
                ...
                if (dep.status === 'open' || gatedByThis) {
                    if (dep.status !== 'blocked') {
                        d.tasks.setStatus(dep.id, 'blocked');
                        d.publish({ type: 'task', taskId: dep.id, status: 'blocked' });
                    }
                    if (!gatedByThis)
                        d.tasks.addLabel(dep.id, `gatedby:${id}`);
                    gated.push(dep.id);
                }
            }
            ...
            if (gated.length > 0) {
                ...
                void d.decisionQueue.enqueue(mission.id, 'review', reviewCtx)
                    ...
                    if (!live || (live.state !== 'active' && live.state !== 'stalled'))
                        return;
