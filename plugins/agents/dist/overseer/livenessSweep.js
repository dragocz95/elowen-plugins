...
export function checkAction(verdict, opts) {
    if (!opts.missionLive || verdict.rationale === 'mission disengaged')
        return { type: 'noop' };
    if (verdict.approve)
        return { type: 'noop' };
    ...
    if (opts.reason === 'progress') {
        if (text)
            return { type: 'steer', text };
        if (verdict.restart)
            return { type: 'restart' };
        return { type: 'noop' };
    }
    if (text)
        return opts.nudges >= opts.nudgeMax ? { type: 'escalate' } : { type: 'nudge', text };
    if (verdict.restart)
        return { type: 'restart' };
 ...
 */
export async function sweepAgentLiveness(d) {
    ...
    const overseerIdle = new Map();
    for (const name of sessions) {
        const info = classifySession(name);
        if (info.role === 'pilot' || info.role === 'advisor')
            continue;
        ...
        if (idle === null) {
            d.tracker.forget(name);
            d.lastProgressAt.delete(name);
            continue;
        }
        if (info.role === 'overseer') {
            if (info.missionId)
                overseerIdle.set(info.missionId, idle);
        ...
        const taskId = d.sessionTaskId(name);
        if (!taskId)
            continue;
        const program = d.programFor(name);
        if (!program)
            continue;
        if (d.hasPrompt(content, program))
            continue; // structured prompt → needs_input, deriver owns it
        if (d.inflightChecks.has(name))
            continue; // a check is already awaiting the overseer
        const reason = idle >= d.workerIdleMs ? 'idle' : 'progress';
        if (reason === 'progress') {
            if (d.progressReviewMs <= 0)
                continue; // routine review disabled (e.g. no overseer exec)
            const last = d.lastProgressAt.get(name);
            // First sight → start the clock and let it work a full interval before its first review.
            if (last === undefined) {
                d.lastProgressAt.set(name, d.now);
                continue;
            }
            if (d.now - last < d.progressReviewMs)
    ...
    for (const e of pending) {
        const l = byMission.get(e.missionId) ?? [];
        l.push(e);
        byMission.set(e.missionId, l);
    }
    for (const [missionId, entries] of byMission) {
        ...
        if (idle === undefined) {
            ...
            if (d.now - since >= d.graceMs) {
                for (const e of entries)
                    if (d.queue.timeout(missionId, e.id))
                        escalated.push(e.id);
        ...
        if (idle >= d.overseerIdleMs) {
            ...
            for (const e of entries)
                if (d.now - e.enqueuedAt >= d.hardMs && d.queue.timeout(missionId, e.id))
                    escalated.push(e.id);
        }
    }
