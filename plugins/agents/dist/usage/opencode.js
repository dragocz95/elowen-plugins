...
function selectOpencodeRow(home, dir, sinceMs, model, nth = 0) {
    ...
    try {
        ...
        const rows = db.prepare(`SELECT id, tokens_input, tokens_output, tokens_reasoning, tokens_cache_read, tokens_cache_write, cost,
        ...
        ).all(resolve(dir), sinceMs - SESSION_MATCH_SKEW_MS);
        if (rows.length === 0)
            return null;
...
export function locateOpencodeSession(home, dir, sinceMs, model, nth = 0) {
...
export function opencodeUsage(home, dir, sinceMs, model, nth = 0) {
