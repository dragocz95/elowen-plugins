...
export function startedMs(task) {
    const lbl = task.labels?.find((l) => l.startsWith('started:'));
    if (lbl) {
        const ms = Number(lbl.slice('started:'.length));
 ...
 *  'codex' | …). */
export function execOf(task, fallback) {
...
export function concurrentRank(task, siblings, fallback, program, since, model) {
    let rank = 0;
    for (const s of siblings) {
        ...
        if (!s.labels?.some((l) => l.startsWith('started:')))
            continue;
        ...
        if (st > since || since - st > SESSION_MATCH_SKEW_MS)
            continue; // only peers that started at/before me, within the window
        if (st < since || (st === since && s.id < task.id))
            rank++; // …and strictly ordered before me
    }
    return rank;
}
