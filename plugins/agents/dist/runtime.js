...
function missionSummaryPrompt(ctx) {
...
function escalateWorker(taskId, d) {
    const task = d.tasks.get(taskId);
    if (!task || task.status === 'blocked')
        return;
    if (task.parent_id && !d.missions.activeForEpic(task.parent_id))
        return; // mission gone → no-op
...
async function restartWorker(task, d) {
    const name = task.labels.find((l) => l.startsWith('agent:'))?.slice('agent:'.length);
    if (name) {
        try {
            captureResumeLabel({ tasks: d.tasks, pathFor: d.usagePathFor, fallback: d.resumeFallback }, task);
        }
        catch (e) {
            d.log.warn(`resume capture failed for ${task.id}`, e);
        }
        await d.tmux.kill(`elowen-${name}`).catch(() => { });
    }
...
async function checkWorker(session, taskId, snapshot, idleMin, reason, d) {
    const task = d.tasks.get(taskId);
    if (!task)
        return;
    ...
    if (!missionId || !(d.missions.get(missionId)?.overseer_exec || d.pluginConfig().overseerExec)) {
        if (reason === 'idle')
            escalateWorker(taskId, d);
        return;
    }
    let verdict;
    try {
        verdict = await d.decisionQueue.enqueue(missionId, 'check', { taskId, session, paneSnapshot: snapshot, idleMin, reason });
    }
    catch (e) {
        d.log.error(`check enqueue failed for ${session}`, e);
        return;
    }
...
function runLivenessSweep(d) {
    void sweepAgentLiveness({
        ...
        if (escalated.length)
            d.log.warn(`liveness sweep escalated ${escalated.length} unanswered decision(s) to a human: ${escalated.join(', ')}`);
        if (checked.length)
            d.log.info(`liveness sweep woke the overseer about ${checked.length} idle worker(s): ${checked.join(', ')}`);
    })
...
export function buildAgentsRuntime(deps) {
    const { tmux, log } = deps;
    ...
    spawn.attachBrainWorker({
        launch: (input) => {
            ...
            if (!worker)
                throw new Error('elowen exec engine not available (brain not configured)');
    ...
    const overseerClient = () => {
        ...
        const relay = deps.config.autopilotRelay();
        if (!relay)
            return null;
    ...
    const usagePathFor = (task) => usagePath(task, (pid) => projects.get(pid)?.path ?? deps.homeProjectPath, (id) => missionGit.worktreeFor(id));
    ...
    const missionIdForSession = (session) => {
    ...
    const decisionRenderer = (taskId) => (name, vars) => deps.prompts.render(name, vars, resolveOwnerId({ tasks, missions, users }, { taskId }));
    const deriver = new Deriver({
        ...
        decideApproval: async (input) => {
            ...
            const recordPrompt = (gated, rationale, confidence) => bus.publish({ type: 'decision', taskId: input.taskId, kind: 'prompt', question: input.question, outcome: gated.approve ? 'approved' : 'escalated', rationale, confidence });
            ...
            if (!inf)
                return noOverseerFallback(input.autonomy);
        ...
        decideQuestion: async (input) => {
            ...
            const recordChoice = (res, rationale, confidence) => bus.publish({ type: 'decision', taskId: input.taskId, kind: 'choice', question: input.question, outcome: res.choiceId ? 'chose' : 'escalated', rationale, confidence, optionLabel: res.choiceId ? input.options.find((o) => o.id === res.choiceId)?.label : undefined });
            if (input.missionId && (missions.get(input.missionId)?.overseer_exec || deps.pluginConfig().overseerExec)) {
    ...
    const reconcileOverseers = async () => {
        ...
        for (const s of live) {
            ...
            if (!activeIds.has(id))
                await tmux.kill(s).catch(() => { });
        }
        for (const m of missions.active()) {
            if (!(m.overseer_exec || deps.pluginConfig().overseerExec))
                continue;
            if (live.has(`elowen-overseer-${m.id}`))
                continue;
            const epic = tasks.get(m.epic_id);
            const proj = epic ? projects.get(epic.project_id) : null;
            if (proj)
                await overseer.start(m.id, proj.id, proj.path);
        }
    };
    ...
    const decisionDeadSince = new Map();
    ...
    const replan = async ({ epicId, goal, exec }) => {
        ...
        if (ok)
            await engine.engage({ epicId, ...engage });
        return ok;
    };
    ...
    const intervals = [
        ...
        { name: 'engine-tick', ms: AGENTS_INTERVAL_MS['engine-tick'], fn: () => { for (const m of missions.live())
                void engine.tick(m.id); } },
        ...
        { name: 'janitor', ms: AGENTS_INTERVAL_MS['janitor'], fn: () => {
                void sweepFinishedSessions({ tmux, taskForSession })
                    .then((reaped) => { if (reaped.length)
                    log.info(`janitor reaped ${reaped.length} finished session(s): ${reaped.join(', ')}`); })
        ...
        { name: 'stuck-detector', ms: AGENTS_INTERVAL_MS['stuck-detector'], fn: () => {
                void sweepStuckTasks({ tmux: liveSessions, tasks, bus, now: clock.now(), graceMs: 120000, maxRelaunch: 2,
                    ...
                    onReap: (t) => { try {
                        captureResumeLabel({ tasks, pathFor: usagePathFor, fallback: resumeFallback }, t);
                    }
                    catch (e) {
                        log.warn(`resume capture failed for stuck task ${t.id}`, e);
                    } } })
                    .then(({ reverted, escalated }) => {
                    if (reverted.length)
                        log.warn(`stuck detector reverted ${reverted.length} dead-agent task(s) to open: ${reverted.join(', ')}`);
                    if (escalated.length)
                        log.error(`stuck detector escalated ${escalated.length} task(s) to blocked after max relaunches: ${escalated.join(', ')}`);
                })
        ...
        { name: 'pr-feedback', ms: AGENTS_INTERVAL_MS['pr-feedback'], fn: () => {
                void sweepPrFeedback({ prs: missionPrs, missions, missionGit, bus, replan })
                    .then((ids) => { if (ids.length)
                    log.info(`PR feedback re-engaged ${ids.length} mission(s): ${ids.join(', ')}`); })
