...
export function registerAdvisorApi(ctx, rt) {
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (!advisor || req.auth.userId === null)
                return json({ running: false, exec: '', session: null });
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (req.path === 'start') {
                if (!advisor || req.auth.userId === null)
                    return json({ error: 'advisor unavailable' }, 503);
                const { exec } = advisorStartSchema.parse(await req.json());
                try {
                    return json(await advisor.start(req.auth.userId, exec), 201);
                }
                catch (e) {
