...
export function registerNotesApi(ctx, rt) {
    const tasks = () => ctx.host.stores().tasks;
    const noteTarget = (raw) => {
        const v = raw ?? '';
        ...
        if (v.startsWith('m-') && tasks().get(v.slice(2)))
            return v.slice(2);
        return v;
    };
    ...
    const gate = (auth, target) => {
        const epic = tasks().get(target);
        if (!epic)
            return json({ error: 'unknown target' }, 404);
        if (!canProject(auth, epic.project_id))
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (!target || !body)
                return json({ error: 'target and body required' }, 400);
            ...
            if (gated)
                return gated;
            if (rt().notes.count(scope, target) >= MAX_NOTES_PER_TARGET)
                return json({ error: 'too many notes' }, 429);
