...
export function registerAsksApi(ctx, rt) {
    ...
    const gate = (auth, id) => {
        ...
        if (!task)
            return { err: json({ error: 'task not found' }, 404) };
        if (!canProject(auth, task.project_id))
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (segs.length === 2 && segs[1] === 'reply') {
                ...
                if (agentForbidden(req.auth, false))
                    return json({ error: 'forbidden' }, 403);
                const askId = decodeURIComponent(segs[0]);
                if (asks().taskFor(askId) !== id)
                    return json({ error: 'no such ask' }, 404);
                const b = askSchema.parse(await req.json());
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => json(asks().pending()
            ...
            .filter((a) => a !== null && canProject(req.auth, a.projectId))),
    });
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            const render = (name, vars) => ctx.host.prompts().render(name, vars, ownerId ?? undefined);
