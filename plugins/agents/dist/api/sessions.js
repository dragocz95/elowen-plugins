...
import { json, canProject, agentForbidden } from './http.js';
...
function sessionAccessibleFor(rt, auth, name) {
    ...
    if (auth.tokenScope !== 'agent' && auth.admin)
        return true;
...
export async function sessionsListPayload(tmux, rt, auth) {
...
export function registerSessionsApi(ctx, rt) {
    const tmux = ctx.host.tmux();
    const tasks = () => ctx.host.stores().tasks;
    const sessionAccessible = (auth, name) => sessionAccessibleFor(rt, auth, name);
    ...
    const execAllowedForUser = (auth, exec) => {
        ...
        if (usersRead.isAdmin(auth.userId))
            return true;
    ...
    const launchManual = async (task, exec) => {
        const r = rt();
        const spec = resolveExecutor(exec ? [`exec:${exec}`] : [], r.resumeFallback);
        const projectId = task.project_id;
        const taskId = task.id;
        if (exec)
            tasks().setExec(taskId, exec); // remember which model ran it — drives the model icon
        ...
        const resolver = {
            projectPath: (pid) => projects.get(pid)?.path ?? ctx.host.stores().homeProject().path,
            worktreeFor: (mid) => r.missionGit.worktreeFor(mid),
        };
        ...
        if (checkoutBusy(resolver, tasks().list({ status: 'in_progress' }), cwd))
            return { ok: false, reason: 'busy', message: 'checkout busy' };
        ...
        if (resume && !tasks().get(taskId)?.resume_note)
            tasks().setResumeNote(taskId, 'Manually restarted — continue from where you left off and finish the task.');
    ...
    const list = async (auth) => json(await sessionsListPayload(tmux, rt, auth));
    const launch = async (req) => {
        const { taskId, exec } = launchSessionSchema.parse(await req.json());
        if (exec && !ctx.host.config().get().allowedExecs.includes(exec))
            return json({ error: 'exec not allowed' }, 400);
        if (exec && !execAllowedForUser(req.auth, exec))
            return json({ error: 'exec not allowed for user' }, 403);
        const task = tasks().get(taskId);
        if (!task)
            return json({ error: 'task not found' }, 404); // don't spawn a phantom agent for a missing task
        ...
        if (!result.ok) {
            if (result.reason === 'busy')
                return json({ error: 'checkout busy' }, 409);
            return json({ error: `spawn failed: ${result.message}` }, 500);
        }
        return json({ session: result.session }, 201);
    };
    const kill = async (auth, name) => {
        if (!sessionAccessible(auth, name))
            return json({ error: 'forbidden' }, 403);
        ...
        if (info.role === 'advisor' && info.userId !== undefined) {
            ...
            if (advisor)
                await advisor.stop(info.userId);
            else
                await tmux.kill(name); // no collaborators in this process — at least kill the pane
            return json({ ok: true });
        }
        ...
        if (terminals.brainWorkerLive(name)) {
            await terminals.brainWorkerAbort(name);
            return json({ ok: true });
        }
        await tmux.kill(name);
        return json({ ok: true });
    };
    const stream = (auth, name) => {
        if (!sessionAccessible(auth, name))
            return json({ error: 'forbidden' }, 403);
        return {
            sse: async (send, signal) => {
                ...
                const timer = setInterval(() => {
                    frame().then(() => { errs = 0; }).catch(() => { if (++errs >= 5)
                        done = true; });
                }, 1000);
                timer.unref?.();
