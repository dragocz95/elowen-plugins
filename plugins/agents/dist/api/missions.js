...
export function missionsListPayload(r, tasks, auth) {
...
export function registerMissionsApi(ctx, rt) {
    ...
    const missionAccessible = (auth, epicId) => {
        if (auth.accessibleProjects === null)
            return true;
        const epic = tasks().get(epicId);
        return !!epic && canProject(auth, epic.project_id);
    };
    const list = (auth) => json(missionsListPayload(rt(), tasks(), auth));
    const detail = (auth, id) => {
        const r = rt();
        const mission = r.missions.get(id);
        if (!mission)
            return json({ error: 'mission not found' }, 404);
        if (!missionAccessible(auth, mission.epic_id))
        ...
        for (const phase of tasks().children(mission.epic_id)) { // the epic's direct phases only
            for (const f of phase.changed_files) {
                ...
                if (cur) {
                    ...
                    totals.set(f.path, { path: f.path, added: f.added, deleted: f.deleted });
            }
        }
        return json([...totals.values()].sort((a, b) => (b.added + b.deleted) - (a.added + a.deleted)));
    };
    const engage = async (req) => {
    ...
    const patch = async (req, id) => {
        const r = rt();
        const mission = r.missions.get(id);
        if (!mission)
            return json({ error: 'mission not found' }, 404);
        if (!missionAccessible(req.auth, mission.epic_id))
            return json({ error: 'forbidden' }, 403);
    ...
    const overseerForbidden = (auth, missionId) => {
        const mission = rt().missions.get(missionId);
        return !!mission && !missionAccessible(auth, mission.epic_id);
    };
    const overseerNext = async (req, id) => {
        if (overseerForbidden(req.auth, id))
            return json({ error: 'forbidden' }, 403);
    ...
    const overseerDecide = async (req, id) => {
        if (overseerForbidden(req.auth, id))
            return json({ error: 'forbidden' }, 403);
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (agentForbidden(req.auth, segs.length === 3 && segs[1] === 'overseer' && segs[2] === 'next'))
                return json({ error: 'forbidden' }, 403);
            if (segs.length === 0)
                return list(req.auth);
            const id = decodeURIComponent(segs[0]);
            if (segs.length === 1)
                return detail(req.auth, id);
            if (segs.length === 2 && segs[1] === 'changed-files')
                return changedFiles(req.auth, id);
            if (segs.length === 3 && segs[1] === 'overseer' && segs[2] === 'next')
                return overseerNext(req, id);
    ...
    ctx.registerApiRoute({
        ...
        handler: async (req) => {
            ...
            if (agentForbidden(req.auth, segs.length === 3 && segs[1] === 'overseer' && segs[2] === 'decide'))
                return json({ error: 'forbidden' }, 403);
            if (segs.length === 0)
                return engage(req);
            const id = decodeURIComponent(segs[0]);
            if (segs.length === 2 && segs[1] === 'pr')
                return openPr(req.auth, id);
            if (segs.length === 2 && segs[1] === 'merge-pr')
                return mergePr(req.auth, id);
            if (segs.length === 3 && segs[1] === 'overseer' && segs[2] === 'decide')
                return overseerDecide(req, id);
            return json({ error: 'not found' }, 404);
        },
    });
    ctx.registerApiRoute({
