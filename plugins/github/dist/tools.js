...
const result = (value) => ({ content: [{ type: 'text', text: typeof value === 'string' ? value : JSON.stringify(value, null, 2) }], details: {} });
const failure = (error) => result(`GitHub: ${error instanceof Error ? error.message : 'The operation failed.'}`);
async function confirmedMutation(ctx, service, action) {
...
export function registerGitHubTools(ctx, service) {
    const tools = [
        defineTool({
        ...
        defineTool({
            ...
            execute: async (_id, input) => { try {
                return result(await service.repositoryStatus(service.currentUserId(), input.projectId));
            }
            catch (error) {
                return failure(error);
            } },
        ...
        defineTool({
            ...
            execute: async (_id, input) => { try {
                return result(await service.listPullRequests(service.currentUserId(), input.projectId, input.state));
            }
            catch (error) {
                return failure(error);
            } },
        ...
        defineTool({
            ...
            execute: async (_id, input) => { try {
                return result(await service.getPullRequest(service.currentUserId(), input.projectId, input.number));
            }
            catch (error) {
                return failure(error);
            } },
        ...
        defineTool({
            ...
            parameters: Type.Object({ projectId: Type.Number(), number: Type.Number() }),
            execute: async (_id, input) => { try {
                return result(await service.pullRequestChecks(service.currentUserId(), input.projectId, input.number));
            }
            catch (error) {
                return failure(error);
            } },
        ...
        defineTool({
            ...
            execute: async (_id, input) => confirmedMutation(ctx, service, { type: 'publish', projectId: input.projectId, sessionId: ctx.currentSessionId() ?? '' }),
        }),
        defineTool({
            ...
            execute: async (_id, input) => confirmedMutation(ctx, service, { type: 'create_pr', projectId: input.projectId, sessionId: ctx.currentSessionId() ?? '', title: input.title, body: input.body, base: input.base }),
        }),
        defineTool({
            ...
            execute: async (_id, input) => confirmedMutation(ctx, service, { type: 'review', ...input }),
        }),
        defineTool({
            ...
            execute: async (_id, input) => confirmedMutation(ctx, service, { type: 'merge', ...input }),
        }),
    ];
