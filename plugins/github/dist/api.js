...
async function objectBody(req) {
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value))
        throw new GitHubPluginError('invalid_request', 400, 'A JSON object is required.');
    return value;
}
...
const route = (ctx, path, method, access, handler) => {
...
export function registerGitHubApi(ctx, service) {
    ...
    route(ctx, 'auth/callback', 'GET', 'user', async (req) => {
        ...
        if (req.query.error) {
            try {
                ...
                destination.searchParams.set('reason', String(response.body.error ?? 'oauth_failed'));
            }
    ...
    route(ctx, 'actions/confirm', 'POST', 'user', async (req) => {
        const input = await objectBody(req);
        return { body: await service.confirm(userId(req), authorizedAction(req, input), text(input.confirmationToken)) };
    });
}
function authorizedAction(req, input) {
...
export function mutationAction(input) {
    ...
    if (type === 'remove_mapping')
        return { type, projectId: requiredProject(input.projectId) };
    if (type === 'publish')
        return { type, projectId: requiredProject(input.projectId), sessionId: requiredText(input.sessionId, 'sessionId') };
    if (type === 'create_pr')
        return {
    ...
    if (type === 'review') {
        ...
        if (event !== 'APPROVE' && event !== 'REQUEST_CHANGES' && event !== 'COMMENT')
            throw new GitHubPluginError('invalid_review', 400, 'Review event must be APPROVE, REQUEST_CHANGES or COMMENT.');
    ...
    if (type === 'merge') {
        ...
        if (method !== undefined && method !== 'squash' && method !== 'merge' && method !== 'rebase')
            throw new GitHubPluginError('invalid_merge_method', 400, 'Merge method must be squash, merge or rebase.');
...
function requiredProject(value) { return requiredNumber(value, 'projectId'); }
function requiredNumber(value, name) {
    const parsed = integer(value);
    if (parsed <= 0)
        throw new GitHubPluginError('invalid_request', 400, `${name} must be a positive integer.`);
    return parsed;
}
function requiredText(value, name) {
    const parsed = text(value);
    if (!parsed)
        throw new GitHubPluginError('invalid_request', 400, `${name} is required.`);
    return parsed;
}
