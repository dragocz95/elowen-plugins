...
export function managedSandbox(ctx) {
...
export async function prepareManagedExecution(input) {
    const prepared = await managedSandbox(input.ctx).prepareExecution({ projectRef: input.project, cwd: input.cwd, command: input.command, leaseKind: 'github' }, { accountUserId: input.accountUserId, roots: [] });
