...
const validHead = (head) => /^[0-9a-f]{40}$/i.test(head);
const trustedGit = async (args, options) => new Promise((resolve, reject) => {
    ...
    child.once('close', code => {
        ...
        terminate();
        if (failed || code !== 0)
            reject(new GitHubPluginError('git_command_failed', 409, 'Git rejected the staged operation.'));
        else
...
export async function stageBundle(bundle, expectedHead, branch) {
    if (!validHead(expectedHead) || !branch || bundle.length > BUNDLE_LIMIT)
        throw new GitHubPluginError('invalid_publish_bundle', 409, 'The proposed publish bundle is invalid.');
    ...
    const run = (args) => exec('/usr/bin/prlimit', [
    ...
    try {
        ...
        if (refs.length !== 1 || refs[0] !== `${expectedHead} refs/heads/${branch}`)
            throw new Error('Bundle does not contain the approved reference');
        ...
        if (imported !== expectedHead)
            throw new Error('Imported commit differs from the approved commit');
        await rm(bundlePath);
...
export async function pushStaged(input, run = trustedGit) {
    ...
    if (actual !== input.expectedHead || !validHead(actual))
        throw new GitHubPluginError('publish_head_changed', 409, 'The approved staged commit changed.');
...
export async function publishManaged(input, publish = pushStaged) {
    const inherited = input.ctx.currentAccess().projectRef;
    const project = input.projectRef ?? inherited;
    if (inherited && project && (inherited.kind !== project.kind || inherited.projectId !== project.projectId))
    ...
    if (project?.kind !== 'managed' || !accountUserId || !validHead(input.expectedHead))
    ...
    try {
        ...
        if (head !== input.expectedHead)
            throw new GitHubPluginError('publish_head_changed', 409, 'The approved project commit changed.');
        ...
        while (true) {
            ...
            if (result.kind !== 'read' || result.totalBytes > BUNDLE_LIMIT || (version !== undefined && result.version !== version))
                throw new Error('Invalid or changed publish bundle');
            version = result.version;
            const bytes = Buffer.from(result.base64, 'base64');
            if (bytes.length > CHUNK || (!bytes.length && offset < result.totalBytes))
                throw new Error('Incomplete publish bundle');
            buffers.push(bytes);
            offset += bytes.length;
        ...
        if (await cleanup())
            throw new GitHubPluginError('publish_cleanup_failed', 500, 'The branch was published, but its temporary publish state could not be removed.');
        return published;
    }
    catch (error) {
        await cleanup();
