...
import type { ProjectGitSnapshot } from 'elowen/dist/git/gitReader.js';
...
import { publishBranch, spawnPrepared, type SpawnPrepared } from './execution.js';
...
export class GitHubService {
  ...
  sessionCredential(input: { accountUserId: number }): { token: string; login: string } | null {
    ...
    if (!Number.isSafeInteger(accountUserId) || accountUserId <= 0) return null;
    const current = this.ctx.currentContributionUserId() ?? this.ctx.currentIdentity()?.elowenUserId ?? null;
    if (current !== accountUserId) return null;
    ...
    try {
      ...
      if (!secret || !/^\S+$/.test(secret.value)) return null;
  ...
  private async repositorySnapshot(project: { id: number; path: string; executionKind?: 'host' | 'managed' }, userId: number): Promise<ProjectGitSnapshot> {
    if (project.executionKind !== 'managed') return this.ctx.host.git().projectSnapshot(project.path);
    const inherited = this.ctx.currentAccess().projectRef;
    if (inherited && (inherited.kind !== 'managed' || inherited.projectId !== project.id)) throw new GitHubPluginError('project_forbidden', 403, 'The repository differs from the selected project.');
    if (this.currentUserId() !== userId) throw new GitHubPluginError('account_mismatch', 403, 'The GitHub account does not belong to the current Elowen account.');
    const git = async (args: string[]) => {
      ...
      if (prepared.mode !== 'managed' || prepared.projectRef?.kind !== 'managed' || prepared.projectRef.projectId !== project.id) { await prepared.lease.release(); throw new GitHubPluginError('project_forbidden', 403, 'The runtime returned a different project.'); }
      return (await (this.spawnPrepared ?? spawnPrepared)(prepared)).stdout;
    };
    const statusText = await git(['status', '--porcelain=v2', '--branch']);
    const status: NonNullable<ProjectGitSnapshot['status']> = { branch: '', head: '', upstream: null, ahead: 0, behind: 0, dirty: 0, untracked: 0, clean: true };
    for (const line of statusText.split('\n')) {
      if (line.startsWith('# branch.head ')) status.branch = line.slice('# branch.head '.length);
      else if (line.startsWith('# branch.oid ')) { const head = line.slice('# branch.oid '.length); status.head = head === '(initial)' ? '' : head; }
      else if (line.startsWith('# branch.upstream ')) status.upstream = line.slice('# branch.upstream '.length);
      else if (/^[12u] /.test(line)) status.dirty++;
      else if (line.startsWith('? ')) status.untracked++;
      const aheadBehind = /^# branch.ab \+(\d+) -(\d+)$/.exec(line);
      if (aheadBehind) { status.ahead = Number(aheadBehind[1]); status.behind = Number(aheadBehind[2]); }
    }
    ...
    for (const line of (await git(['remote', '-v'])).split('\n')) {
      ...
      const name = match[1]!;
      let safe = match[2]!;
      try { const url = new URL(safe); url.username = ''; url.password = ''; url.search = ''; url.hash = ''; safe = url.href; }
      catch { safe = safe.replace(/^[^@]+@([^:]+:)/, 'git@$1'); }
      const remote = remotes.get(name) ?? { name, fetchUrl: '', pushUrl: '' };
      if (match[3] === 'fetch') remote.fetchUrl = safe; else remote.pushUrl = safe;
  ...
  private async publishState(userId: number, projectId: number, sessionId: string): Promise<{
    workspace: { workspaceId: string; path: string; branch: string; baseRef: string }; mapping: ProjectMapping; base: GitHubRepository; head: string; projectRef?: ProjectExecutionRef;
  }> {
    const project = this.project(userId, projectId);
    if (project.executionKind === 'managed') {
      const inherited = this.ctx.currentAccess().projectRef;
      if (inherited && (inherited.kind !== 'managed' || inherited.projectId !== projectId)) throw new GitHubPluginError('project_forbidden', 403, 'Select the managed project before publishing.');
      const selected = { kind: 'managed' as const, projectId };
      const provider = this.ctx.control('sandbox');
      if (!provider) throw new GitHubPluginError('sandbox_unavailable', 503, 'Project environment unavailable.');
      ...
      if (!path) throw new GitHubPluginError('active_workspace_required', 409, 'Select the managed project repository or one of its worktrees before publishing.');
      const git = async (args: string[]) => {
        const live = this.ctx.control('sandbox');
        if (!live) throw new GitHubPluginError('sandbox_unavailable', 503, 'Project environment unavailable.');
        ...
        if (prepared.mode !== 'managed' || prepared.projectRef?.kind !== 'managed' || prepared.projectRef.projectId !== projectId) { await prepared.lease.release(); throw new GitHubPluginError('project_forbidden', 403, 'The runtime returned a different project.'); }
      ...
      if (!branch || !/^[a-f0-9]{40}$/i.test(head)) throw new GitHubPluginError('publish_requires_commit', 409, 'Select a committed branch before publishing.');
      const mapping = this.requireMapping(userId, projectId);
