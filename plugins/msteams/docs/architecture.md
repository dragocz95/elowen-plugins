...
5. `type === 'message'`:
   ...
   through the Connector, never as the HTTP response. `tests/msteams.test.ts:405` pins the
...
`tests/msteams.test.ts:370` runs a real key pair against a local JWKS server and pins all
...
- `sweepStaleAsks` (`:286`) drops timed-out `AskUserQuestion` cards. It runs here, on *every* inbound
  ...
  of the process. Pinned by `tests/msteams.test.ts:634`.
...
exists. Pinned by `tests/msteams.test.ts:429`.
...
becomes context for a later session (`tests/msteams.test.ts:536`). Bot-control commands are
...
conversation (`tests/msteams.test.ts:527`). A plugin **prompt macro** is deliberately not
...
3. **Identity** (`:394` → `senderIds`, `lib/ids.mjs:18`): the sender's `aadObjectId`, their `29:` channel
   ...
   operator copies out of a Teams deep link. Pinned by `tests/msteams.test.ts:158`.
4. **Role policy** (`:395` → `accessFor`, `:338`): the **first** policy whose `roleId` matches any of
   ...
   line. `tests/msteams.test.ts:193` pins the absence of a reply specifically; it is not
...
- `this.handler(src, text, onEvent)` is the seam into the host. `src` carries `platform`, `userId`
  ...
  conversation — pinned deliberately by `tests/msteams.test.ts:498`.
...
- On failure (`:454-462`): the interval is cleared, the error is **logged and** replied. Both, on
  ...
  Pinned by `tests/msteams.test.ts:623`.
...
in a conversation must not steer a privileged session. `tests/msteams.test.ts:511` asserts
...
Pinned by `tests/msteams.test.ts:443` (both shapes ring, a stranger stays text) and `:457`
...
Pinned character-for-character by `tests/msteams.test.ts:467`.
...
2. `onAskAction` (`:860`) drops unknown tokens, settles an expired one to a "question expired" card
   ...
   someone else" (`:869-876`). Pinned by `tests/msteams.test.ts:252` for the *refusal*.
...
- Every `/display` axis offers an explicit `default` value (`:1007`), which deletes the per-chat override
  ...
  Pinned by `tests/msteams.test.ts:584`.
...
2. **No target** → a warning naming both `notifyConversationId` and the job's `notifyChannelId`, and the
   ...
   already paid a real turn for; `tests/msteams.test.ts:613` pins the warning.
...
4. If the target is not a known conversation, `notifyConversationFor` (`:791`) resolves it: an ambiguous
   ...
   `tests/msteams.test.ts:797`); a unique person gets their 1:1 chat opened; an unseen
...
- **Layer 1** is `PeopleDirectory.resolve` (`lib/directory.mjs:111`): `email`/`aadObjectId`/`userId`/
  ...
  **more than one candidate is refused with the list**, never guessed (`lib/directory.mjs:130-136`,
  `lib/adapter.mjs:697-699`). Pinned by `tests/msteams.test.ts:686`.
...
- `conversationForPerson` returns the remembered `conv` when there is one, else opens a 1:1 chat via
  ...
  **same** conversation for the same bot/user pair, so it is idempotent; the id is still remembered both
  ...
  Pinned by `tests/msteams.test.ts:695`.
