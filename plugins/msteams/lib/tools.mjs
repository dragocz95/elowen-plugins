...
export function registerTools(ctx, adapter) {
  ...
  //
  // CURATED tools — look up a person, read a roster, send a message, hand over a file — require the
  // linked Elowen account to be an administrator. A room role never supplies that permission.
  //
  // RAW connector access is the exception and stays with the operator: TeamsApi drives the bot
  // credentials directly and can reconfigure or drain anything they touch. An administrator account is
  // still not necessarily the instance operator, so the raw credential boundary remains separate.
  //
  ...
  const adminGate = (name) => {
    ...
    throw new Error(`${name} requires a linked Elowen administrator account. Room roles never grant tool permissions; ask an Elowen administrator to update your account.`);
  };
  const ownerGate = (name) => {
    if (ctx.currentIdentity?.()?.owner === true) return;
    throw new Error(`${name} is reserved for the instance operator — a single account (the first admin), not a role. An "admin": true role policy does NOT grant it, because this tool drives the raw bot credentials. Ask the operator to run it, or use TeamsSend / TeamsMessagePerson / TeamsSendFile, which an administrator account may call.`);
  };
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'This posts immediately and visibly to real people and cannot be unsent, so confirm the recipient before calling it. It requires a linked Elowen administrator account, and it works only through the Bot Connector API — the bot must already be in that conversation.',
    ].join(' '),
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'Delivery may go through the recipient own Elowen agent when their account is mapped, otherwise it is sent as a plain direct message; the result says which path was taken and reports partial delivery when Teams accepted only some parts. An unsolicited direct message reaches a real person immediately and cannot be recalled, and the tool requires a linked Elowen administrator account.',
    ].join(' '),
    parameters: Type.Object({
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'The directory only covers people the bot has actually seen, mostly through conversation rosters, so someone missing here may still exist in the tenant — reading a channel roster with TeamsMembers teaches the bot about its members. At most 25 matches are shown, and the tool requires an administrator account.',
    ].join(' '),
    parameters: Type.Object({
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'It is read-only and posts nothing. It works only for conversations the bot participates in, and the type shows as "unknown" when the bot has not yet observed enough about that chat. Requires an administrator account.',
    ].join(' '),
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'The listing is capped at 50 members, with the remainder reported as a count, and it only works for conversations the bot participates in. Requires an administrator account.',
    ].join(' '),
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'Both ids are required: the member is looked up within that one conversation, so an id from a different chat will not resolve. It is read-only and sends nothing, and it requires an administrator account.',
    ].join(' '),
    parameters: Type.Object({
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'The listing shows raw ids and counts, not conversation titles or message content, so use TeamsChatInfo to see what a given id actually is. It is read-only, covers only the current service host, and requires an administrator account.',
    ].join(' '),
    parameters: Type.Object({
  ...
  ctx.registerTool(defineTool({
    ...
    description: [
      ...
      'Files cannot be sent into a channel or a group chat — that is a Teams limitation, so post a link there instead. The offer reaches a real person and cannot be withdrawn, and the tool requires an administrator account.',
    ].join(' '),
    parameters: Type.Object({
