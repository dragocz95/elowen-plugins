...
export function ProjectGitPanel({ project }: { project: Project }) {
  ...
  // Only shared host vocabulary comes from `t` now; every label this panel owns lives in the plugin
  // manifest, so the core deleting its project strings cannot blank this panel again.
  ...
  if (git.isError) {
    ...
    return <p role="alert" className="flex flex-wrap items-center gap-2 py-4 text-xs text-muted-foreground">{message}<Button variant="ghost" onClick={() => { void git.refetch(); }}>{host.common.retry}</Button></p>;
  }
  if (!git.data) return null;
  if (!git.data.isRepo) return <div className="py-4"><Badge tone="muted">{s.notGit}</Badge></div>;

  return (
    <>
      {git.data.status ? (
        <section className="border-b border-border/70 py-4">
          <h3 className="mb-3 flex items-center gap-2 text-xs font-semibold text-foreground"><FolderGit2 size={14} className="text-muted-foreground" aria-hidden />{s.gitLabel}</h3>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {git.data.status.clean
              ? <Badge tone="success"><CheckCircle2 size={11} className="mr-1" aria-hidden />{s.gitClean}</Badge>
              : <button type="button" onClick={() => navigate(editorUrl(project.id, { working: '1' }))} title={s.viewChanges} className="rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70"><Badge tone="warning"><AlertTriangle size={11} className="mr-1" aria-hidden />{s.gitDirty.replace('{count}', String(git.data.status.dirty))}</Badge></button>}
      ...
      {git.data.branches.length > 0 ? (
        <section className="border-b border-border/70 py-4">
          <h3 className="mb-3 flex items-center gap-2 text-xs font-semibold text-foreground"><GitBranch size={14} className="text-muted-foreground" aria-hidden />{s.gitBranches}</h3>
      ...
      {git.data.commits.length > 0 ? (
        <section className="py-4">
          <h3 className="mb-2 flex items-center gap-2 text-xs font-semibold text-foreground"><GitCommitHorizontal size={14} className="text-muted-foreground" aria-hidden />{s.gitCommits}</h3>
          <EntityList>
