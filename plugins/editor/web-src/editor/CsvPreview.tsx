...
export function CsvPreview({ source, invalidLabel, limitedLabel }: { source: string; invalidLabel: string; limitedLabel: string }) {
  ...
  if (fatalErrors.length > 0) {
    ...
    return <p className="p-4 text-center text-sm text-destructive">{invalidLabel}: {first.message}</p>;
  }
  const rows = parsed.data.slice(0, MAX_ROWS);
  const width = rows.reduce((max, row) => Math.max(max, row.length), 0);
  return (
    <div className="h-full overflow-auto bg-background p-4">
      {parsed.data.length > MAX_ROWS ? <p className="mb-3 text-xs text-muted-foreground">{limitedLabel.replace('{count}', String(MAX_ROWS))}</p> : null}
