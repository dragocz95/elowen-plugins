...
export function ImagePreview({ projectId, root, path }: { projectId: number; root: EditorRoot; path: string }) {
  ...
  useEffect(() => {
    ...
    setUrl(null);
    setFailed(false);
    fetch(editorFileUrl(projectId, 'raw', root, { path }), { credentials: 'same-origin' })
