...
export async function editorExecute(ctx: PluginContext, projectId: number, accountUserId: number, command: SandboxExecutionCommand, cwd: string): Promise<string> {
  ...
  let failure: unknown;
  try {
    if (prepared.mode !== 'managed' || prepared.projectRef.projectId !== projectId || typeof prepared.start !== 'function' || typeof prepared.cancel !== 'function') throw new Error('managed execution unavailable');
    ...
    const collect = (chunk: Buffer, stdout: boolean): void => {
      bytes += chunk.length;
      if (bytes > 8 * 1024 * 1024) interrupt(new Error('project command output too large'));
      else if (stdout) output.push(chunk);
    };
    session.stdout.on('data', (chunk: Buffer) => collect(chunk, true));
    session.stderr.on('data', (chunk: Buffer) => collect(chunk, false));
    session.stdin.end();
    const result = await Promise.race([session.closed, interrupted]);
    if (result.code !== 0) throw new Error('project command failed');
    return prepared.sanitizeOutput(Buffer.concat(output).toString('utf8'));
  } catch (error) {
    failure = error;
    try { await prepared.cancel?.(); } catch (cleanup) { failure = new AggregateError([failure, cleanup], 'Project command cancellation failed'); }
    throw failure;
  } finally {
    clearTimeout(timer);
    clearInterval(heartbeat);
    try { await prepared.lease.release(); } catch (cleanup) { throw new AggregateError([...(failure ? [failure] : []), cleanup], 'Project command cleanup failed'); }
  }
}
