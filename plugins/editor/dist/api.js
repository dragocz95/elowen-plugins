...
const unknownRoot = () => ({ status: 400, body: { error: 'this project does not have that root' } });
function projectFor(ctx, req) {
    const root = parseEditorRoot(req.query.root);
    if (!root)
        return { status: 400, body: { error: 'unknown editor root' } };
    ...
    if (project.executionKind === 'managed')
        return { path: project.path, hostSystem: false, managed: { projectId: id, slug: project.slug, root } };
...
export function registerEditorApi(ctx) {
    ...
    const guardFor = (target) => (target.hostSystem ? safeSystemPath : hostSafe);
    const route = (rootMount, method, handler) => {
        ctx.registerApiRoute({ rootMount, path: '', method, access: 'user', handler: async (req) => {
                ...
                if (isResponse(project))
                    return project;
                if (project.managed)
                    return managedEditorRequest(ctx, req, project.managed, rootMount, method);
    ...
    route('/projects/:id/commit/:hash', 'GET', async (req, project) => {
        try {
            return { body: { diff: await projectCommitDiff(project.path, req.params.hash ?? ''), files: await projectCommitFiles(project.path, req.params.hash ?? '') } };
        }
        catch (error) {
            return fileError(error);
        }
    ...
    route('/projects/:id/commits', 'GET', async (req, project) => {
        ...
        try {
            return { body: { commits: await projectCommitLog(project.path, limit) } };
        }
        catch (error) {
            return fileError(error);
        }
    ...
    route('/projects/:id/changed', 'GET', async (_req, project) => {
        try {
            return { body: { changed: await projectChangedFiles(project.path) } };
        }
        catch (error) {
            return fileError(error);
        }
    ...
    route('/projects/:id/changes', 'GET', async (_req, project) => {
        try {
            return { body: { diff: await projectWorkingDiff(project.path) } };
        }
        catch (error) {
            return fileError(error);
        }
