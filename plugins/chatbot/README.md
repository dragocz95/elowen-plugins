...
**Acting on the page.** The allowlist lives on the server, in `p_chatbot_action_rules`: a row names an origin, a path prefix and an action, with its own per-turn cap and whether the visitor must confirm it. The most specific matching rule wins, a kind a matching rule does not name is refused, and the rules of one origin are the policy of that origin. An origin with no rule at all is governed by the implicit policy its allowlist entry already implies, so a freshly configured chatbot is useful before its first rule is written. The model reaches all of this through one tool, `ChatbotPageAction`, which takes a `snapshotId`, an `action`, a `targetId` and, where the kind carries one, a `value`: `read`, `focus`, `click`, `fill`, `select`, `scroll` and `request_submit`. A target id is meaningful only inside the snapshot that issued it, a click on a submit-capable element is refused outright, and submitting a form is its own kind. The tool refuses to run outside a live chatbot visitor turn, and what it is told back is what the plugin recorded — the value a `read` found, the page's own refusal, or the fact that it expired.
