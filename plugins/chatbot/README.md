...
**Acting on the page.** The allowlist lives on the server. A `page-agent` action is only ever performed when a server-approved frame names it for THIS turn's description: `read`, `focus`, `click`, `fill`, `select`, `scroll`, and `request_submit`. A target id is meaningful only inside the snapshot that issued it, a click on a submit-capable element is refused outright, and submitting a form is its own kind: the widget shows the visitor what is about to be sent and only a click THEY make can confirm it, because the confirm listener ignores any event the browser did not mark as trusted. The confirmation is recorded on the server before the form is sent — a confirmation the deployment could not store is one it never gave, so nothing is submitted.

The widget reports every action's outcome (and every refusal) to `POST /hooks/chatbot/v1/turns/:id/actions/:id/result` and the visitor's answer to `POST …/actions/:id/confirmation`. Those two paths are part of the v1 contract but are **not served yet**: the model-side half that asks for an action (the tool that writes a `page_action` and waits for its outcome) is the next step, and until it lands a widget that meets a deployment without them gives up reporting after the first `404` rather than retrying something nobody serves.
