...
export function createAdminApi(deps) {
    ...
    const privacyUnresolved = () => ({ status: 409, body: { error: 'privacy_policy_unresolved' } });
    ...
    const requireBot = (chatbotUserId) => {
        ...
        if (!account)
            return { status: 404, body: { error: 'account_unknown' } };
        return { row };
    };
    const isRefusal = (value) => 'status' in value;
    const readChatbotUserId = (raw) => {
    ...
    return {
        ...
        async create(auth, body) {
            ...
            if (parsed.value.sensitiveMode)
                return privacyUnresolved();
        ...
        async conversations(auth, query) {
            const refusal = requireAdmin(auth);
            if (refusal)
                return refusal;
        ...
        async visitors(auth, query) {
            ...
            return {
                status: 200,
                body: { visitors: rows.slice(0, VISITORS_LIMIT), truncated: rows.length > VISITORS_LIMIT },
            };
        },
        ...
        async stats(auth, query) {
            ...
            if (spanDays > STATS_MAX_DAYS)
                return { status: 400, body: { error: 'invalid_request', detail: `the window may span at most ${STATS_MAX_DAYS} days` } };
