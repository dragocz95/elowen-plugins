...
export function allowedOfferUrl(value, origins) {
    if (value.length > OFFER_LIMITS.url)
        return false;
    try {
...
export function readOffer(value, origins) {
    if (!object(value, ['choices', 'links', 'cards']))
        return null;
    const offer = {};
    for (const key of ['choices', 'links', 'cards']) {
        const items = value[key];
        if (items === undefined)
            continue;
        if (!Array.isArray(items) || items.length > OFFER_LIMITS[key])
            return null;
        if (key === 'choices') {
            const choices = [];
            for (const item of items) {
                if (!object(item, ['label', 'reply']) || !short(item.label, OFFER_LIMITS.label)
                    || (item.reply !== undefined && !short(item.reply, OFFER_LIMITS.reply)))
                    return null;
            ...
            for (const item of items) {
                if (!object(item, ['label', 'url']) || !short(item.label, OFFER_LIMITS.label)
                    || typeof item.url !== 'string' || !allowedOfferUrl(item.url, origins))
                    return null;
            ...
            for (const item of items) {
                if (!object(item, ['title', 'subtitle', 'price', 'meta', 'imageUrl', 'action'])
                    ...
                    || (item.imageUrl !== undefined && (typeof item.imageUrl !== 'string' || !allowedOfferUrl(item.imageUrl, origins))))
                    return null;
                let action;
