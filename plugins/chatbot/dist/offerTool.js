...
export function registerOfferTool(deps) {
    ...
    ctx.registerTool(defineTool({
        ...
        execute: async (_id, input) => {
            ...
            if (!live.ok)
                throw new Error(`ChatbotOffer only works in a running chatbot visitor turn (${live.reason}).`);
            const checked = validateOffer(input, store.originsOf(live.chatbotUserId));
            if (!checked.ok)
                return { content: [{ type: 'text', text: checked.error }], details: { status: 'refused' } };
