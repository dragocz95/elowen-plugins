...
const AVATAR_LIMITS = { timeoutMs: AVATAR_FETCH_TIMEOUT_MS, maxBytes: AVATAR_MAX_BYTES };
/** Fetch one avatar through the host's validated transport. */
export async function fetchAvatarBytes(source, upstream, limits = AVATAR_LIMITS) {
    let url;
    try {
        url = new URL(source);
    }
 ...
 *  on it. */
export function createAvatarFetcher(transport, limits = AVATAR_LIMITS) {
    return async (source) => {
        let upstream;
        try {
            upstream = transport();
...
function imageMediaType(header) {
