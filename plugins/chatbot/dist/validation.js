...
export function validateFeedback(body) {
    ...
    return { ok: true, value: { rating: rating, comment: trimmed || null } };
}
...
export function validateActionResult(body, kind) {
...
export function validateBotCreate(body) {
    const outer = strictObject(body, ['chatbotUserId', 'displayName', 'origins', 'limits', 'sensitiveMode', 'maySubmitForms'], ['chatbotUserId']);
    if (!outer.ok)
        return outer;
    ...
    if (!maySubmitForms.ok)
        return maySubmitForms;
    return {
        ok: true,
        value: {
...
export function validateBotPatch(body) {
    const outer = strictObject(body, ['chatbotUserId', 'expectedUpdatedAt', 'displayName', 'origins', 'limits', 'action', 'sensitiveMode', 'maySubmitForms'], ['chatbotUserId', 'expectedUpdatedAt', 'displayName', 'origins', 'maySubmitForms']);
    if (!outer.ok)
        return outer;
    const userId = outer.value.chatbotUserId;
    if (typeof userId !== 'number' || !Number.isSafeInteger(userId) || userId <= 0)
