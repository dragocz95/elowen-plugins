...
function imageMime(bytes: Uint8Array): string | null {
  return VISITOR_IMAGE_FORMATS.find((format) => format.signatures.some((alternative) =>
    alternative.every((part) => bytes.length >= part.offset + part.bytes.length
...
export function verifiedImageStream(body: ReadableStream<Uint8Array>, size: number): ReadableStream<Uint8Array> {
  if (!Number.isSafeInteger(size) || size < VISITOR_IMAGE_MIN_BYTES || size > VISITOR_IMAGE_MAX_BYTES) throw new Error('invalid_image_size');
