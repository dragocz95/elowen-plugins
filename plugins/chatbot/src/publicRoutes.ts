...
import type { ChatbotClientOrigin, ChatbotConversationFiles, ChatbotHookRequest, ChatbotStores } from './coreSeams.js';
...
import { isCanonicalUuid, validateActionDecision, validateActionResult, validatePublicBotRequest, validateTurnSubmission, validateFeedback, type Validated } from './validation.js';
...
export function createPublicRoute(deps: PublicRouteDeps) {
  ...
  const trustedOrigin = (req: ChatbotHookRequest): ChatbotClientOrigin | null => {
  ...
  const presentedToken = (req: ChatbotHookRequest): { bot: BotRow; visitorId: string } | Reply => {
    ...
    if (!presented) return reply(401, { error: VISITOR_CREDENTIAL_ERRORS.required });
    const verified = verifyVisitorToken({ secret: deps.secret(), token: presented, nowMs: now().getTime() });
    if (!verified.ok) return reply(401, { error: VISITOR_CREDENTIAL_ERRORS.invalid });
    const row = store.token(verified.payload.jti);
    if (!row || row.revoked_at !== null) return reply(401, { error: VISITOR_CREDENTIAL_ERRORS.invalid });
    if (!sameHash(hashToken(presented), row.token_hash)) return reply(401, { error: VISITOR_CREDENTIAL_ERRORS.invalid });
    ...
    if (!visitor || visitor.revoked_at !== null) return reply(401, { error: VISITOR_CREDENTIAL_ERRORS.invalid });
  ...
  const publicBotRequest = async (req: ChatbotHookRequest, origin: string): Promise<{ bot: BotRow } | { reply: Reply }> => {
    const notJson = requireJsonBody(req, origin);
    if (notJson) return { reply: notJson };
    const body = await readJson(req);
    if (!body.ok) return { reply: reply(400, { error: 'invalid_request', detail: body.error }, corsHeaders(origin)) };
    const parsed = validatePublicBotRequest(body.value);
    if (!parsed.ok) return { reply: reply(400, { error: 'invalid_request', detail: parsed.error }, corsHeaders(origin)) };
    ...
    if (!allowed.ok) return { reply: reply(403, { error: 'origin_not_allowed' }) };
  ...
  const handleBootstrap = async (req: ChatbotHookRequest, origin: string): Promise<Reply> => {
  ...
  const handleUpload = async (req: ChatbotHookRequest, origin: string, requestOrigin: ChatbotClientOrigin): Promise<Reply> => {
    const admitted = presentedToken(req);
    if ('status' in admitted) return admitted;
    ...
    if (!clientTurnId || !isCanonicalUuid(clientTurnId) || typeof name !== 'string'
      ...
      || !req.stream) return reply(400, { error: 'invalid_image' }, headers);
    ...
    if (limited) return reply(429, { error: 'rate_limited' }, headers);
    if (!deps.files) return reply(503, { error: 'bot_unavailable' }, headers);
    try {
      ...
      if (!accepted) return reply(409, { error: 'upload_not_available' }, headers);
  ...
  const handleTurn = async (req: ChatbotHookRequest, origin: string, requestOrigin: ChatbotClientOrigin): Promise<Reply> => {
    const admitted = presentedToken(req);
    if ('status' in admitted) return admitted;
    const allowed = checkAllowedOrigin(origin, store.originsOf(admitted.bot.chatbot_user_id));
    if (!allowed.ok) return reply(403, { error: 'origin_not_allowed' });
    ...
    if (existing) return turnReceipt(existing, origin);
    ...
    if (!outcome.ok) {
      if (outcome.reason === 'duplicate') return turnReceipt(outcome.turn, origin);
  ...
  const handleFile = (req: ChatbotHookRequest, origin: string, turnId: string,
    kind: string, storedName: string): Reply => {
    const admitted = presentedToken(req);
    if ('status' in admitted) return admitted;
    if (!checkAllowedOrigin(origin, store.originsOf(admitted.bot.chatbot_user_id)).ok)
      return reply(403, { error: 'origin_not_allowed' });
    const headers = { ...corsHeaders(origin), 'cache-control': 'private, no-store', 'x-content-type-options': 'nosniff' };
    const missing = (): Reply => reply(404, { error: 'not_found' }, headers);
    if (!isCanonicalUuid(turnId) || (kind !== 'image' && kind !== 'file')) return missing();
    const turn = store.turn(turnId);
    if (!turn || turn.visitor_id !== admitted.visitorId || turn.chatbot_user_id !== admitted.bot.chatbot_user_id
      || !turn.core_session_id) return missing();
    if (!store.attachmentEventsOf(turnId).some((event) => event.kind === kind && event.storedName === storedName)) return missing();
    if (!deps.files) return reply(503, { error: 'bot_unavailable' }, headers);
  ...
  const handleFeedback = async (req: ChatbotHookRequest, origin: string, requestOrigin: ChatbotClientOrigin, turnId: string): Promise<Reply> => {
    const admitted = presentedToken(req);
    if ('status' in admitted) return admitted;
    if (!checkAllowedOrigin(origin, store.originsOf(admitted.bot.chatbot_user_id)).ok) {
      return reply(403, { error: 'origin_not_allowed' });
    ...
    if (limited) return admissionReply(limited, origin);
    ...
    if (!saved) return reply(404, { error: 'not_found' }, corsHeaders(origin));
  ...
  const handleActionReport = async (
    ...
    if (kind === 'result') {
      ...
      return actionReportReply(
        actions.reportResult({ turn, actionId, outcome: parsed.value.outcome, detail: parsed.value.detail, origin }),
        origin,
      );
    }
  ...
  const handlePreflight = (req: ChatbotHookRequest, origin: string): Reply => {
  ...
  const handleHandoff = async (req: ChatbotHookRequest, origin: string): Promise<Reply> => {
    const headers = { ...corsHeaders(origin), 'cache-control': 'no-store' };
    const bad = () => reply(400, { error: 'invalid_request' }, headers);
    const notJson = requireJsonBody(req, origin);
    if (notJson) return notJson;
    const body = await readJson(req);
    if (!body.ok || !body.value || typeof body.value !== 'object' || Array.isArray(body.value)) return bad();
    const value = body.value as Record<string, unknown>;
    if (value.schemaVersion !== PUBLIC_SCHEMA_VERSION) return bad();
    if ('code' in value) {
      if (Object.keys(value).some(key => !['schemaVersion', 'bot', 'code'].includes(key))
        ...
        || typeof value.bot !== 'string') return bad();
      const bot = store.botByPublicId(value.bot);
      const refused = admitBot(bot);
      if (refused) return refused;
      if (!checkAllowedOrigin(origin, store.originsOf(bot!.chatbot_user_id)).ok) return reply(403, { error: 'origin_not_allowed' });
      const blocked = blockedReply(bot!);
      if (blocked) return blocked;
      const actionId = store.consumeHandoff(hashToken(value.code), origin, iso(), bot!.chatbot_user_id);
      if (!actionId) return reply(403, { error: 'invalid_handoff' }, headers);
      ...
      if (!result.ok) return reply(409, { error: 'action_closed' }, headers);
    ...
    if (Object.keys(value).some(key => !['schemaVersion', 'turnId', 'actionId'].includes(key))
      ...
      || typeof value.actionId !== 'string' || !isCanonicalUuid(value.actionId)) return bad();
    const admitted = presentedToken(req);
    if ('status' in admitted) return admitted;
    if (!checkAllowedOrigin(origin, store.originsOf(admitted.bot.chatbot_user_id)).ok) return reply(403, { error: 'origin_not_allowed' });
    ...
    if (!turn || turn.visitor_id !== admitted.visitorId || turn.chatbot_user_id !== admitted.bot.chatbot_user_id
      ...
      || action.expires_at <= iso()) return reply(404, { error: 'not_found' }, headers);
    const request = actionRequestPayload(action);
    let url: URL;
    try { url = new URL(request.value ?? ''); } catch { return bad(); }
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password
      || !checkAllowedOrigin(url.origin, store.originsOf(admitted.bot.chatbot_user_id)).ok) return reply(403, { error: 'origin_not_allowed' }, headers);
  ...
  return async function handlePublicRequest(req: ChatbotHookRequest): Promise<Reply> {
    ...
    if (!requestOrigin) return reply(403, { error: 'trusted_origin_required' });
    ...
    if (req.method === 'OPTIONS') return handlePreflight(req, origin);

    if (req.method === 'POST' && path === PUBLIC_PATHS.handoff) return handleHandoff(req, origin);
    if (req.method === 'POST' && path === PUBLIC_PATHS.bootstrap) return handleBootstrap(req, origin);
    ...
    if (req.method === 'POST' && path === PUBLIC_PATHS.uploads) return handleUpload(req, origin, requestOrigin);
...
function turnReceipt(turn: TurnRow, origin: string): Reply {
...
function admissionReply(outcome: AdmissionRefusal, origin: string): Reply {
  const headers = corsHeaders(origin);
...
/** Everything one admission can refuse with, minus the one outcome that is not a refusal at all. */
type AdmissionRefusal = Extract<Exclude<AdmissionOutcome, { reason: 'duplicate' }>, { ok: false }>;
...
function actionReportReply(outcome: ActionReportOutcome, origin: string): Reply {
  ...
  if (outcome.reason === 'invalid_result') return reply(400, { error: 'invalid_request' }, corsHeaders(origin));
