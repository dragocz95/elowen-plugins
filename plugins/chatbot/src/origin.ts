import type { ChatbotClientOrigin, ChatbotHookRequest } from './coreSeams.js';
...
export function normalizeOrigin(raw: string): string {
  ...
  if (value === '') throw new OriginError('empty');
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  if (url.protocol !== 'https:' && url.protocol !== 'http:') throw new OriginError('unsupported_scheme');
  if (url.username !== '' || url.password !== '') throw new OriginError('credentials_not_allowed');
  if (url.pathname !== '/' || url.search !== '' || url.hash !== '') throw new OriginError('path_not_allowed');
  if (url.host === '') throw new OriginError('host_required');
 ...
 *  site can be. */
export function isUsableOrigin(origin: string): boolean {
  try {
...
export function checkAllowedOrigin(headerOrigin: string | undefined, allowed: readonly string[]): OriginCheck {
  if (typeof headerOrigin !== 'string') return { ok: false, reason: 'origin_not_allowed' };
  const requested = headerOrigin.trim();
  if (requested === '') return { ok: false, reason: 'origin_not_allowed' };
...
export function readRequestOrigin(req: ChatbotHookRequest): ChatbotClientOrigin | null {
  const origin: unknown = req.origin;
  if (typeof origin !== 'object' || origin === null) return null;
  const candidate = origin as Record<string, unknown>;
  if (typeof candidate.value !== 'string' || candidate.value === '') return null;
  if (candidate.kind !== 'ip' && candidate.kind !== 'local' && candidate.kind !== 'internal' && candidate.kind !== 'platform') return null;
...
export function isTrustedRequestOrigin(origin: ChatbotClientOrigin | null): boolean {
