...
export interface VisitorTokenPayload {
  v: 1;
  /** The chatbot's PUBLIC id. A token is bound to one chatbot and is refused for any other. */
  bot: string;
  /** The visitor id this token speaks for. */
  sub: string;
  /** Token id: the handle a rotation revokes and an audit names. */
  jti: string;
...
export type VisitorTokenVerification =
  ...
  | { ok: false; reason: VisitorTokenFailure };
...
const signatureOf = (secret: string, signed: string): Buffer =>
  createHmac('sha256', secret).update(signed, 'utf8').digest();

export function mintVisitorToken(secret: string, payload: VisitorTokenPayload): string {
...
export function verifyVisitorToken(input: {
  ...
  const [, encoded, providedSignature] = parts as [string, string, string];
  ...
  if (!isVisitorTokenPayload(parsed)) return { ok: false, reason: 'shape' };
...
function isVisitorTokenPayload(value: unknown): value is VisitorTokenPayload {
  ...
  if (Object.keys(candidate).length !== 6) return false;
...
export function readAuthorizationToken(headers: Record<string, string>): string | null {
  const raw = headers.authorization ?? headers.Authorization;
  if (typeof raw !== 'string') return null;
  const [scheme, value, ...rest] = raw.trim().split(/\s+/);
  if (rest.length > 0 || scheme !== VISITOR_AUTHORIZATION_SCHEME || !value) return null;
  return value;
}
