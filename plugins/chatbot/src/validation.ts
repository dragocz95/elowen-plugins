...
export interface BotCreatePayload {
  ...
  prompt: string;
  origins: string[];
  /** A draft may be registered without any limit filled in; what is carried is validated, and the rest stays
   *  unset until an administrator decides it. Enabling is where completeness is required. */
...
export interface BotPatchPayload {
  ...
  prompt: string;
  origins: string[];
  /** The limits this write carries. A field left out is not part of the write; a field carried as null is
   *  the number the administrator has not decided. */
  limits: Partial<LimitValues>;
  /** Whether this write ASKS for the sensitive-data mode, which this version refuses (see adminApi). */
  sensitiveMode: boolean;
