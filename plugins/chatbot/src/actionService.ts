...
import { resolveActionRule, type ActionRuleRefusal } from './actionRules.js';
...
/** Why an action never became one. The policy's own reasons, plus the three facts only the server can
 *  establish: the turn recorded no page to act on, the rules do not allow this here, and a rule asks for a
 *  confirmation this version cannot carry. */
export type ActionRequestRefusal = ActionRefusal | ActionRuleRefusal | 'no_page_state';
...
export class PageActionService {
  ...
  async request(input: {
    ...
    if (!verdict.ok) return this.refuse(input, verdict.reason);
