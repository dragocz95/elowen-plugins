/** The v2 public contract: every value the widget, the public hook and the page-action policy have to
 ...
 *
 *  `v2` is the protocol AND the asset version. A compatible change may replace the served asset; a
 *  breaking one gets a new mount rather than runtime detection of two shapes inside one handler. */

/** The one mount this plugin declares. Every public path below is a remainder under it. */
export const PUBLIC_MOUNT = 'v2';
...
export const ACTION_KINDS = ['snapshot', 'navigate', 'read', 'focus', 'click', 'fill', 'select', 'scroll', 'request_submit'] as const;
...
export const CONFIRMATION_ACTION_KIND: ActionKind = 'request_submit';
...
export const PUBLIC_PATHS = {
  ...
  actionDecision: (turnId: string, actionId: string): string =>
    `${PUBLIC_SEGMENTS.turns}/${turnId}/${PUBLIC_SEGMENTS.actions}/${actionId}/${PUBLIC_SEGMENTS.confirmation}`,
  widget: PUBLIC_SEGMENTS.widget,
} as const;

/** The outcomes a widget reports for a performed action. */
export const ACTION_OUTCOMES = ['done', 'error', 'denied'] as const;
...
export const ACTION_DECISIONS = ['confirm', 'decline'] as const;
...
export type PageFailureDetail = (typeof PAGE_FAILURE_DETAILS)[number];

/** Whether an action kind is only ever performed after the visitor themselves confirmed it. */
export function requiresVisitorConfirmation(kind: ActionKind): boolean {
