...
import type { AssertPublished, DeckNavigationLayout, DeckNavGroup, PluginConfigDraft } from 'elowen-plugin-ui-kit';
export type { DeckNavItem, DeckNavGroup } from 'elowen-plugin-ui-kit';
...
interface ChatbotHooks {
   ...
   *  its cache. That is what lets five independently mounted sections read ONE register: a bundle that
...
interface ChatbotComponents {
  ...
  DataTableCell: ComponentType<{ children?: ReactNode; header?: boolean; priority?: 'always' | 'mobile' | 'wide'; lines?: 1 | 'auto'; title?: string; className?: string }>;
  DataTableChevronCell: ComponentType<Record<string, never>>;
  ...
  EntityRow: ComponentType<{ children?: ReactNode }>;
 ...
 *  modal, with five sections switching inside it.
 *
 ...
 *
 *  All five routes resolve to the SAME component on purpose: React then keeps the deck — its column, its
 *  strip and its scroll position — mounted across a section change, and only the content pane is
 *  replaced. Five distinct page components would rebuild the frame on every click.
 *
