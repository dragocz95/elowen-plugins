...
export function SecuritySettings({ origins, rules, disabled, onChange }: {
  ...
  return (
    ...
    >
      {rules.length === 0 ? (
        <C.SettingsRow label={s.rulesEmpty} description={s.rulesEmptyHint} />
      ) : (
        rules.map((rule) => (
          <C.SettingsRow
            key={actionRuleKey(rule)}
      ...
      <div className="mt-2 grid gap-3 rounded-xl border border-border bg-muted/20 p-3 sm:grid-cols-2">
