...
export function npmInstallGlobal(packages, prefix = lspPrefixDir()) {
    try {
...
export function npmUninstallGlobal(packages, prefix = lspPrefixDir()) {
    return runNpm(['uninstall', '-g', '--prefix', prefix, ...packages], 'uninstalled');
}
function runNpm(args, okDetail) {
    return new Promise((resolve) => {
        ...
        child.on('exit', (code) => {
            if (code === 0) {
                resolve({ ok: true, detail: okDetail });
                return;
            }
