...
export function registerLspSetup(ctx) {
    if (typeof ctx.registerSetupStep !== 'function')
        return;
    ctx.registerSetupStep({ id: 'lsp', run: runLspSetup });
}
async function runLspSetup(setup) {
    ...
    let response;
    try {
        response = await setup.request('GET', '/brain/lsp');
    }
    catch {
    ...
    try {
        const installed = await setup.request('POST', '/brain/lsp/install', { command: row.command });
        if (installed.ok) {
