...
export class LspManager {
    ...
    async statusAsync() { return this.status(); }
    ...
    async cancellable(operation, signal) {
        if (!signal)
            return { done: true, value: await operation() };
        if (signal.aborted)
            return { done: false };
    ...
    async checkFile(path, boundary, signal) {
        ...
        return this.queueDiagnostic(key, async () => {
            ...
            if (!this.enabled || this.epoch !== epoch)
                return { path, diagnostics: [], skipped: 'disabled' };
            if (signal?.aborted)
                return { path, language, diagnostics: [], skipped: 'cancelled' };
            let text;
            try {
            ...
            if (entry === 'crash-looping')
                return { path, language, server: spec.label, diagnostics: [], skipped: 'crash-looping', maxRestarts: this.maxRestarts };
            if (!entry)
            ...
            try {
                ...
                const verdict = await this.cancellable(() => entry.client.diagnose(path, text, language, timeoutMs, this.settleMs), signal);
                if (!verdict.done) {
    ...
    async withClient(path, boundary, op, signal) {
        if (!this.enabled)
        ...
        if (signal?.aborted)
            return { ok: false, reason: 'cancelled', language };
        let text;
        try {
            text = await this.readFile(path);
        }
        ...
        const entry = raced.value;
        if (entry === 'crash-looping')
            return { ok: false, reason: 'crash-looping', language, server: spec.label, maxRestarts: this.maxRestarts };
        if (!entry)
    ...
    async workspaceSymbol(query, boundary, signal) {
        ...
        if (inScope.length === 0) {
            ...
            const entry = raced.value;
            if (entry === 'crash-looping')
                return { ok: false, reason: 'crash-looping', maxRestarts: this.maxRestarts };
            if (!entry)
                return { ok: false, reason: 'no-server-installed' };
            inScope = [entry];
        }
    ...
    async spawnAnyClientFor(root, signal) {
        let capped = false;
    ...
    async clientFor(spec, root, signal) {
        if (signal?.aborted)
            return null;
        const key = this.keyFor(spec, root);
        ...
        if (existing)
            this.retire(existing, 'crash'); // a crashed/exited server client — evict and respawn below
        ...
        if ((this.restarts.get(key) ?? 0) >= this.maxRestarts)
            return 'crash-looping';
        const epoch = this.epoch;
        const transport = await this.spawnFn(spec, root);
        if (!transport)
            return null;
        if (!this.enabled || this.epoch !== epoch || signal?.aborted) {
            transport.dispose();
            return null;
        }
...
export function formatCheckResult(r) {
    ...
    if (r.skipped === 'crash-looping')
        return `The ${r.server ?? r.language} language server exceeded max crash recovery attempts (${r.maxRestarts ?? 3}) — no diagnostics, and it will NOT be restarted again until LSP is toggled off and on (/lsp).`;
    ...
    if (r.skipped === 'cancelled')
        return 'LSP: the check was cancelled before the server answered — re-issue it if still needed.';
