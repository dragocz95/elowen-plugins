...
export function renderTaskContext(tasks) {
  ...
  return [
    ...
    '    TaskCreate adds NEW work, taking the whole plan as one call with every task in its tasks array, each task carrying at least a subject and a description. Put prerequisites in blockedBy: an existing task uses its id such as "3", while a sibling in the same call uses "$1" for the first task. Write blockedBy: ["$1"] for a sibling; blockedBy: ["3", "$1"] mixes an existing task and a sibling. This is the only sibling dependency syntax. TaskCreate returns the new ids. TaskUpdate only changes work that already exists and never creates it. TaskDelete removes selected existing tasks atomically and updates the Todo panel once.',
