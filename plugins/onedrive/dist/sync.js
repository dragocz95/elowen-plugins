...
import { buildIgnore, containedIn, containedInEventually, gitIgnoredAmong, hashFile, normalizeIgnorePatterns, scanLocal, scanManaged, TRASH_DIR, openMirrorFile, } from './scan.js';
...
export class SyncEngine {
    ...
    async underRootLock(root, work) {
        ...
        try {
            ...
            if (this.rootLocks.get(key) === settled)
    ...
    async syncLink(link, drive, confirmDeletions) {
        ...
        try {
            ...
            if (managed)
                await this.underRootLock(base, () => this.applyManagedCycle(link, drive, confirmDeletions, managed, lease));
            else
    ...
    async applyManagedCycle(link, drive, confirmDeletions, fs, lease) {
        ...
        const paths = new Set([...scan.files.keys(), ...baseline.keys(), ...listing.files.keys()]);
        const planned = [];
        let accounted = 0;
        for (const rel of paths) {
            ...
            if (item && item.size > maxBytes)
                continue;
            const remote = item ? { present: true, itemId: item.id, etag: item.etag, size: item.size } : { present: false };
            if (known)
                accounted += 1;
        ...
        if (!current || !current.enabled || current.remoteItemId !== link.remoteItemId || current.remoteDriveId !== link.remoteDriveId)
            return;
        ...
        for (const entry of planned) {
            ...
            try {
                if (entry.action === 'upload' && entry.local.present) {
                    ...
                    try {
                        if (entry.version && file.version !== entry.version)
                            throw new Error(`Managed file changed while it was being scanned: ${entry.rel}`);
                    ...
                    if (await fs.stat(entry.rel))
                        continue;
                    if (entry.known?.remoteItemId)
                        await drive.remove(entry.known.remoteItemId, entry.known.remoteEtag || undefined);
                    this.deps.store.dropItem(link.id, entry.rel);
                }
                else if (entry.action === 'trashLocal' && entry.known?.remoteItemId) {
                    if (await drive.stillExists(entry.known.remoteItemId))
                        continue;
