    ...
    async syncLink(link, drive) {
        ...
        try {
            ...
            if (deletions > DELETION_FLOOR && deletions > baseline.size * DELETION_CEILING) {
                ...
                this.deps.store.setStatus(link.id, 'error', `${deletions} mirrored files disappeared locally at once. Nothing was deleted in OneDrive; check that the project folder is complete, then sync again.`);
                return;
            }
