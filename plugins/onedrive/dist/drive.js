...
export class Drive {
    ...
    async itemAt(folderId, rel) {
        try {
            const raw = await this.graph.json('GET', `${this.itemAddress(folderId, rel)}?$select=id,name,eTag,cTag,size,file,folder,parentReference`);
    ...
    async upload(folderId, rel, file, ifMatch, expectNew = false) {
        ...
        try {
            ...
            if (existing !== null && existing.size > 0)
                throw new RemoteItemAppearedError(rel, existing);
