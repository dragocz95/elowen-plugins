...
export function normalizeSubpath(value) {
    ...
    for (let depth = 1; depth <= segments.length; depth += 1) {
        ...
        if (isFloorIgnored(prefix) || isFloorIgnored(`${prefix}/probe`))
            return null;
    }
...
async function gitFiles(root) {
    try {
        ...
        const text = `${error.stderr ?? ''} ${message(error)}`.toLowerCase();
...
export async function gitIgnoredAmong(root, paths) {
    const out = new Set();
    if (paths.length === 0)
    ...
    for (const rel of Buffer.concat(chunks).toString('utf8').split('\0').filter(Boolean))
        out.add(rel);
...
export async function scanManaged(fs, options) {
    ...
    for (const entry of candidates) {
        ...
        if (bucket)
            ...
            byRemoteKey.set(remoteKey(entry.rel), [entry.rel]);
    }
    const colliding = new Set();
    for (const bucket of byRemoteKey.values())
        if (bucket.length > 1)
    ...
    const skipped = [];
    for (const entry of candidates) {
        if (colliding.has(entry.rel)) {
            skipped.push({ rel: entry.rel, reason: 'collision' });
            continue;
        }
        if (entry.size > options.maxBytes) {
            skipped.push({ rel: entry.rel, reason: 'too-large' });
            continue;
        }
        if (options.now - entry.mtimeMs < options.settleMs) {
            skipped.push({ rel: entry.rel, reason: 'settling' });
            continue;
        }
