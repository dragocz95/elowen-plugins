...
export const uploadTimeoutMs = (size: number): number =>
  UPLOAD_GRACE_MS + Math.ceil(Math.max(size, 0) / UPLOAD_FLOOR_BYTES_PER_SECOND) * 1_000;
...
export class Drive {
  ...
  private itemAddress(folderId: string, rel: string): string {
  ...
  async upload(
    ...
    try {
      ...
      if (await this.itemIdAt(folderId, rel) !== null) throw error;
  ...
  async sha256(itemId: string): Promise<string> {
    const { body } = await this.graph.binary(`${this.base()}/items/${encodeURIComponent(itemId)}/content`, {
      maxBytes: 1024 * 1024 * 1024,
    });
