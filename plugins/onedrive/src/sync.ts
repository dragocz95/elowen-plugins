...
export class SyncEngine {
   ...
   *  held back by this. */
  private readonly deferredUntil = new Map<number, number>();
      ...
      for (const entry of planned) {
        ...
        try {
          ...
          if (error instanceof RemoteItemAppearedError && local.present) {
            ...
            bytes += await this.resolveUntrackedPair(
              drive, link, root, rel, local,
              { present: true, itemId: error.item.id, etag: error.item.etag, size: error.item.size },
              !known,
            );
            continue;
          }
          ...
          this.deps.log.warn(
            `onedrive mirror ${link.id}: ${entry.action} ${rel}: ${message(error)}${graphDetail ? ` [${graphDetail}]` : ''}`,
          );
          failures.push(rel);
        }
      }
  ...
  private async resolveUntrackedPair(
...
function isEnvironmentPending(error: unknown): boolean {
  return (error as { code?: unknown } | null)?.code === 'environment_pending';
}
