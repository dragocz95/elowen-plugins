...
export function register(published: PluginContext): void {
  ...
  const settings = (): SyncSettings => ({
    ...
    extraIgnore: normalizeIgnorePatterns(
      typeof ctx.config.extraIgnore === 'string' || Array.isArray(ctx.config.extraIgnore)
  ...
  const withinBase = (base: string, subpath: string): string | null => {
