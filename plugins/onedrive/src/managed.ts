...
export class ManagedMirror {
  ...
  async git(args: readonly string[]): Promise<{ stdout: string; stderr: string; code: number | null }> {
    ...
    try {
      ...
      if (result.code === null) throw new Error(`Managed Git command ended on ${result.signal}`);
