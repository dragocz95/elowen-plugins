...
describe('the visitor token is the only visitor authority', () => {
  ...
  it('refuses a token whose signed chatbot, token row and visitor row do not all agree', async () => {
    ...
    current.store.createBot({ chatbotUserId: 13, publicId: `cbt_${'c'.repeat(24)}`, displayName: 'Škola', prompt: '', origins: [SITE], limits: TEST_LIMITS, actionRules: [], now: new Date().toISOString() });
...
describe('the message path', () => {
  ...
  it('gives two chatbots different acting accounts and different sessions', async () => {
    ...
    second.store.createBot({ chatbotUserId: 13, publicId: `cbt_${'c'.repeat(24)}`, displayName: 'Škola', prompt: '', origins: [SITE], limits: TEST_LIMITS, actionRules: [], now: new Date().toISOString() });
