...
describe('msteams proactive notify + app package', () => {
  ...
  it('uploads an image generated by a cron job through the ctx.notify delivery path', async () => {
    const { adapter, state, calls } = await makeAdapter({ notifyConversationId: 'a:conv1' });
    state.patch('a:conv1', { ref: { serviceUrl: 'https://smba.test/emea' } });
    const root = mkdtempSync(join(tmpdir(), 'cron-teams-image-'));
    const imageDir = join(root, 'chat-images');
    const name = `${'a'.repeat(64)}.png`;
    try {
      mkdirSync(imageDir);
      writeFileSync(join(imageDir, name), Buffer.from('PNG'));
      adapter.imageDir = imageDir;
      mkdirSync(join(root, 'cronjob'));
      writeFileSync(join(root, 'cronjob/jobs.json'), JSON.stringify([{
        id: 'scheduled-image', name: 'illustration', schedule: 'every 5m', prompt: 'draw it',
      ...
      const registry = await loadPlugins({
        dirs: [join(repoRoot, 'plugins')], enabled: ['cronjob'], dataRoot: root, logger: log,
        pluginDb: pluginDbFor(root),
        host: { stores: { conversationsRead: stubConversationDirectory() } } as never,
        notify: (text, channelId, images) => adapter.notify(text, channelId, undefined, images),
      });
      const cron = registry.platforms[0] as unknown as {
        listen(handler: Function): void;
        control(api: { relay: Function }): void;
        tick(): Promise<void>;
      };
      wireCronHost(cron, async (_src: unknown, _text: string, onEvent?: (event: unknown) => void) => {
        onEvent?.({ type: 'image', ref: `/api/brain/chat-images/${name}`, caption: 'Generated' });
      ...
      expect(sends[0]!.args[2]).toMatchObject({
        text: 'Generated',
        attachments: [{ name, contentType: 'image/png' }],
      });
      expect(sends[1]!.args[2]).toMatchObject({ text: expect.stringContaining('Scheduled result') });
