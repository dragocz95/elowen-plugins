...
const coreUiKitApiVersion = /PLUGIN_UI_API_VERSION:\s*(\d+)\b/.exec(readFileSync(coreUiKitTypesPath, 'utf8'))?.[1];
if (coreUiKitApiVersion !== undefined && Number(coreUiKitApiVersion) < 16) {
  throw new Error(`[copied-helper-parity] ELOWEN_CORE_ROOT exposes plugin UI API ${coreUiKitApiVersion}, below the contract's floor: ${coreRoot}`);
}
