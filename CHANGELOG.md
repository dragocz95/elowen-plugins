...
- Its administration surface lists chatbots with their person, Project, prompt, allowed origins and action rules, opens the conversations of one chatbot with the transcript, and shows statistics with spend per origin. The look is configurable too: one editor sets the colours, the corner radius, the panel size and corner, the greeting, the avatar, the quick buttons and light or dark mode, with the real panel drawn beside the controls so a change is visible before it is saved.
